...
The figures of the 2026-09-19 run at `origin/main @ d814500` (U1 of plan records-refresh; load 3.13 6.16 6.24 on 10 cores at run start; CI run 35446265780 green on d814500). Kept because two of the seven commits of `d814500..20298cc` changed what the live table describes, in two different ways: #706 (PR #707, 20298cca) moved the toolchain (TypeScript 6 to 7, vite 8.0 to 8.3, Node 24 in CI), touching `package.json`, `package-lock.json` and the four workflows and no file under `test/`; #699 (PR #702, 9a44f685) touched only files under `test/`, adding `repo/gate-report.mjs` to `TESTS` and with it the 47 to 48 step, and no toolchain file. So these figures are history to compare against, not a range to grade against. The rows are headed differently from the live table on purpose: `baseline-agrees-check.sh` reads the first row that starts with the live head.

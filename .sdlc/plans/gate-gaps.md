...
| 2026-09-21 | revision 4, conductor ruling on U1's base: U1 is cut from `plan/gate-split` (tip 557b6c0c), not from `origin/main`, because its only file `test/engine/anchor.mjs` does not exist on main and #713 splits it into a FULL sweep and a SAMPLED leg; a U1 written against the unsplit file would needle a shape nobody ships. G0 is waived for U1 on the same reasoning as gate-split Q8, and the landing order stays #681, then #713, then #715, stated in U1's handoff. If gate-split's tip moves (U5's merge, then U6b), the unit rebases and re-runs its gates rather than assuming. |

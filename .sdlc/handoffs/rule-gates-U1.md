...
- `figma/plugin/ui.html`: regenerated by `npm test` (embeds the changed `styles.css`); committed
  separately so the tree stays byte-stable after a test run.
...
`12897b98` on `unit/rg-U1`.

## Disagreed with the plan

Nothing in the design or the criteria text disagreed. One thing the plan's U1 steps did not
mention explicitly: `npm test` regenerates `figma/plugin/ui.html` from the changed `styles.css`,
so it had to be committed as its own follow-up commit to satisfy P1's byte-stable requirement :

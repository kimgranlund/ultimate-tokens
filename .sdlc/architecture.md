...
- Doc drift (§8, measured at `d814500`): 12 rows, 1 drifted, 10 hold, 1 undetermined.
...
| DD8 | `.claude/CLAUDE.md:28` "the 10 documented color formats: CSS/OKLCH/JSON/DTCG/UI3/Tailwind/shadcn" | `src/engine/exports.js`: nine single-purpose format emitters (`exportCSS`, `exportOKLCH`, `exportJSON`, `exportDTCG`, `exportUI3`, `exportTailwind`, `exportShadcn`, `exportPanda`, `exportRadix`) plus one aggregator, `exportAll` | undetermined: the file's own header comment (`exports.js:1,4-6`) lists the same nine named formats "plus the exportAll aggregator", worded as if the aggregator is separate from the count, not the tenth format. The 10th slot is unresolved from the code alone; a human ruling on what the tenth format is would settle it |

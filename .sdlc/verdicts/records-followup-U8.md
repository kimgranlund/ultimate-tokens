...
| 6 | the branding gate passes at the head and does scan `.sdlc/` | 🟢 | `node test/repo/branding.mjs` in the head clone prints `branding: clean (481 files scanned)`, exit `0`. `test/repo/branding.mjs:41` skips only `.git`, `node_modules`, `dist`, `other` and the worktree directories, so `.sdlc/` is in scope | clone `neg` with `NONOUN` appended to `.sdlc/handoffs/records-followup-U8.md`: the gate prints `✗ .sdlc/handoffs/records-followup-U8.md: contains "NONOUN"` and `FAIL: 1 branding violation(s) across 481 files`. The gate reads this unit's own new file |

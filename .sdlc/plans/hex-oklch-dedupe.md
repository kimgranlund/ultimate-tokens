...
| U1-4 | the agreement assertion is in `npm test` and bites | `node test/ui/model.mjs > "$F/m.log" 2>&1; echo "exit $?"; grep -c 'keyOklch' "$F/m.log"; grep -o 'keyOklch[^,]*subjects [0-9]*' "$F/m.log"` (the pass line prints its subject count; the builder states the exact wording it chose and the verifier reads it) | `exit 0`, `1` or more, a subject count of `16` (the default kit's anchored families) or the count the builder constructed, at least `1` | in the clone, the U1-2 edit (`hexToOklch("#000000")` for `keyOklch`, `keyHex` kept): `exit 1`, a FAIL line naming the first palette whose `keyOklch` is not `hexToOklch(anchor)`; `ui/model.mjs` is in `TESTS` on main, so the same red is `npm test`'s | `test/ui/model.mjs` has `0` lines naming `keyOklch` on 3e1483e4 and on 65b3bf27 |

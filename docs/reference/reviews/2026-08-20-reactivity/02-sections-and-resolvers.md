...
**Geometry** — mirrors Typography exactly, method-for-method: `geomAnalysisCards(view)` (geometry.js:573, same "unused view" comment) → `this._activeGeomScale()`; `renderGeomInspector` (geometry.js:707) binds `this.doc.geometry`; exports via `_geomScaleFor("base")`/`_geomModeScales()` (drawer.js:48,69,398-419; apply-gate.js:279).

...
priority: P1
lane: docs
size: S + S + M (U1 S = 1 point, U2 S = 1 point, U3 M = 2 points)
labels: kind:chore · size:S · lane:docs · mode:multi
written: 2026-09-18
amended: 2026-09-18 (U2, C31); 2026-09-19 (U3, re-measure at 20298cc after #706)
...
Scope wall. Only these paths change: `.sdlc/baseline.md` (U1 whole; U3 whole again, keeping U1's figures as a prior set), `.sdlc/adapter.md` (U1; U3 the three §1 Time cells only), `.sdlc/debt.md` (the P3 row and one note), `.sdlc/architecture.md` (U1: one note; U2: one rerun note, one Counts bullet, one appended `## 8. Doc drift` section; U3: the rerun note, the Counts bullet, §8's head sentence and rows, and the §6 K17 row, all in place), `.sdlc/checks/baseline-agrees-check.sh` (new), `.gitignore`, `.sdlc/verdicts/architecture.md` (U2: one appended `## Pass 5` section, written by the verifier), plus each unit's handoff (`.sdlc/handoffs/records-refresh-U<n>.md`), each unit's verdict (`.sdlc/verdicts/records-refresh-U<n>.md`), and the Orchestrator's board and checklist. Every plant U2 needs under `src/`, `test/`, `figma/`, `mcp/`, `scripts/` happens in a throwaway clone, never in a worktree of this branch. `.claude/CLAUDE.md` does not change: it carries no test count (measured: the stale-count sweep in U1 row 5 finds nothing under `.claude/`), and adapter C9 allows an edit there only inside a unit with a criterion for it. `.sdlc/survey.md` is a dated snapshot, already corrected in 33ee3b1. Verdicts, handoffs, archived plans, tickets, and questions are history and keep their numbers.
...
| 9 | each not-re-derived record opens with one dated staleness note that quotes the verdict faithfully, states true facts, and carries the re-check rule; nothing else in either file changed except debt row P3 (amended 2026-09-19: and the §6 K17 row, U3's one in-place edit to a pre-plan line, excluded the way P3 is; the command read `grep -cE '^-[^-]'` on `architecture.md` when U1 and U2 were graded) | `for f in architecture debt; do awk '/^## /{exit} /^Staleness note \(graded 2026-09-18\)\./{n++} END{print n+0}' .sdlc/$f.md; grep -c 'git diff --stat f9e20c5 HEAD' .sdlc/$f.md; done; grep -c '18 of the 72' .sdlc/architecture.md; grep -c '16 of the 96' .sdlc/debt.md; grep -c '18 of the 72.*16 of the 96' .sdlc/verdicts/survey.md; git merge-base --is-ancestor f9e20c5 origin/main; echo "ancestor $?"; git diff --name-only f9e20c5 180eca0 -- src test scripts mcp figma \| wc -l; MB=$(git merge-base origin/main HEAD); git diff $MB -- .sdlc/architecture.md \| grep -E '^-[^-]' \| grep -vc '^-[\|] K17 [\|]'; git diff $MB -- .sdlc/debt.md \| grep -E '^-[^-]' \| grep -vc '^-[\|] P3 [\|]'` | four lines of `1` (note count and rule count, per file), then `1`, `1`, `1`, `ancestor 1`, `10`, `0`, `0` | at 746f93d the four note counts and the two quote counts print `0`. `git merge-base --is-ancestor 180eca0 origin/main` prints `ancestor 0`, so the ancestry test can say yes. The rule's own command is not vacuous, measured: `git diff --stat f9e20c5 HEAD -- src/engine/tonal.js` prints `113 insertions(+), 18 deletions(-)` and the same on `src/engine/hct.js` prints nothing |
...
2. Quiet host, as U1 step 1: `uptime` and `sysctl -n hw.ncpu`; a 1-minute load above the core count (10) means tell the Orchestrator and wait. No other seat's gate in flight: the Orchestrator confirms Lane B idle on the board before dispatch, and before each run the builder checks `pgrep -fl 'test/run.mjs|vite build|smoke.mjs'` prints nothing. `node --version` and, after step 4's `npm ci`, `npm ls typescript vite --depth=0` go in the handoff.
...
10. Run U3 rows 1 to 9, then P1, P4, P5 and, as regression on the files U3 edits, U1-1 to U1-9 and U2-1 to U2-6 as written (with the two amended commands). Hand off.
...
`.sdlc/handoffs/records-refresh-U3.md`: the adopt-hygiene handoff shape plus three tables and a line `measured at 20298cc`. Runs: `| # | command | load (1 min) before | load (1 min) after | exit | wall (s) | last line | git status lines |`, rows `1` to `9` for the three commands in step order (`npm ci` is a prose line above the table). Rows: `| DD | cited paths | changed in d814500..20298cc | own result at 20298cc | state before | state after |`, 56 rows, column three `y` or `n` from `git diff --name-only d814500 20298cc` against the row's cells. Controls: `| K | own result at 20298cc | pass 5 cell | delta | plant | hits |`, 18 rows, no empty cell.

#### Criteria

Rows 1 to 9 are graded by the U3 verifier. Values marked "at d46ae48a" were measured by the planner on 2026-09-19 in the survey-refresh worktree; planted values in a throwaway shared clone of it. `F` is `$CLAUDE_JOB_DIR/tmp`.
...
| 2 | a full rerun at `20298cc`: three runs of three commands, the front matter names the head and the loads, the prior figures live only in the prior set with its three rows, and #706 is named once as the reason | `grep -cE '^[\|] .npm (test\|run build\|run smoke). [\|] 3/3 [\|] 0 [\|]' .sdlc/baseline.md; grep -c '^[\|] .npm test. [\|]' .sdlc/baseline.md; grep -c '^ref: origin/main @ 20298cc$' .sdlc/baseline.md; grep -cE '^host: .*load [0-9.]+ [0-9.]+ [0-9.]+ on 10 cores' .sdlc/baseline.md; awk '/^## Prior set/{exit} {print}' .sdlc/baseline.md \| grep -cE 'd814500\|35446265780\|3777\.8\|47 test files\|63\.54'; awk '/^## Prior set \(d814500/,0' .sdlc/baseline.md \| grep -cE '^[\|] .npm (test\|run build\|run smoke). \(prior, d814500\) [\|] 3/3 [\|] 0 [\|]'; grep -c '#706' .sdlc/baseline.md` | `3`, `1`, `1`, `1`, `0`, `3`, `1` | at d46ae48a: `3`, `1`, `0`, `1`, `4`, `0`, `0`: the head line, the prior-set rows and the reason bite. A copy with the prior section deleted prints `0` on the sixth line; a copy whose live test row still reads `47 test files` prints `1` on the fifth; a copy naming #706 in the front matter as well prints `2` on the last. Measured in the clone: the prior rows' heading keeps the first count at `3` and the second at `1` |
...
| 4 | §8 was re-checked row by row at `20298cc`: no DD row still names `d814500`, the three head mentions read `20298cc` (U2-2 at the new head), the row check exits 0 and is unedited, and the handoff shows every row re-checked with the rows citing a #706-touched path marked | `awk '/^## 8\. Doc drift/,0' .sdlc/architecture.md \| grep -E '^[\|] DD' \| grep -c d814500; H=$(git merge-base origin/main HEAD \| cut -c1-7); echo $H; grep -oE '^ref: .*@ [0-9a-f]+' .sdlc/baseline.md \| grep -oE '[0-9a-f]{7}'; awk '/^## 8\. Doc drift/,0' .sdlc/architecture.md \| grep -oE 'Measured at .[0-9a-f]{7}' \| grep -oE '[0-9a-f]{7}$'; awk '/^## /{exit} /^Rerun note \(/' .sdlc/architecture.md \| grep -oE 'rerun at .[0-9a-f]{7}' \| grep -oE '[0-9a-f]{7}$'; awk '/^## 7\. Counts/,/^## 8\./' .sdlc/architecture.md \| grep -oE 'measured at .[0-9a-f]{7}' \| grep -oE '[0-9a-f]{7}$'; sh .sdlc/checks/doc-drift-rows-check.sh \| tail -1; diff <(awk '/^\140\140\140sh doc-drift$/{f=1;next} /^\140\140\140$/{f=0} f' .sdlc/plans/records-refresh.md) .sdlc/checks/doc-drift-rows-check.sh && echo same; HF=.sdlc/handoffs/records-refresh-U3.md; grep -oE '^[\|] DD[0-9]+ [\|]' $HF \| sort -u \| wc -l; grep -E '^[\|] DD[0-9]+ [\|]' $HF \| awk -F'[\|]' '{for(i=2;i<NF;i++) if($i ~ /^ *$/) c++} END{print c+0}'; for p in $(git diff --name-only d814500 20298cc); do awk '/^## 8\. Doc drift/,0' .sdlc/architecture.md \| grep -E '^[\|] DD' \| grep -F -- "$p" \| awk -F'[\|]' '{gsub(/ /,"",$2); print $2}'; done \| sort -u \| wc -l; grep -E '^[\|] DD[0-9]+ [\|]' $HF \| grep -c '[\|] y [\|]'` | `0`; `20298cc` five times; `rows N drifted D holds H undetermined U bad 0`; `same`; `N` (the same N); `0`; `M`; `M` or more (M is 14 at d46ae48a) | at d46ae48a: `24`; `20298cc` then `d814500` four times; `rows 56 drifted 11 holds 45 undetermined 0 bad 0`; `same`; the handoff is absent, so the last four lines are `0`, `0`, `14`, `0`. In the clone, `sed 's/d814500/20298cc/g'` over `architecture.md` alone turns the first line to `0` and the sha lines to five of `20298cc` without any re-check, which is what the handoff's per-row results and the `y` count exist for: the sha can be swapped, a row's own result at `20298cc` cannot. A handoff copy with DD19's third cell `n` prints `13` on the last line |
...
| 6 | K17's exception list is corrected, the corrected control prints nothing at `20298cc`, and it still catches an unregistered test file | `grep -E '^[\|] K17 [\|]' .sdlc/architecture.md \| grep -c 'gate-report-(clean.[\|]mismatch.[\|]singlequote)'; grep -E '^[\|] K17 [\|]' .sdlc/architecture.md \| grep -c '7 unlisted files'; git ls-files test \| grep "\.mjs$" \| sed "s#^test/##" \| while read f; do grep -q "\"$f\"" test/run.mjs \|\| echo "$f"; done \| grep -vxE "run.mjs\|smoke/smoke.mjs\|ui/counts.mjs\|gate-report.mjs\|repo/fixtures/gate-report-(clean\|mismatch\|singlequote).mjs" \| wc -l` | `1`, `1`, `0` | at d46ae48a: `0`, `0` (the row reads `3 unlisted files`), and the row's own filter prints the four names, the pass 5 🔴. The first pattern's `.[|]` matches the cell's own `\|` pipe escape (measured: the same shape on the row's existing three-name filter prints `1`), so the row is read as written, not as typed. In the clone with a tracked `test/engine/zzz.mjs`, the third command prints `1` and the name `engine/zzz.mjs`: the widened filter still discriminates. A filter widened to `repo/fixtures/.*` instead would swallow a future `repo/fixtures/real-test.mjs`, so the three fixture names are spelled out |
| 7 | adapter: the time ranges follow the new baseline, and the in-place edits stay inside the §1 gate table against the new merge base (U1-6 at `20298cc`) | `MB=$(git merge-base origin/main HEAD); git diff $MB -- .sdlc/adapter.md \| grep -E '^-[^-]' \| grep -vcE '^-[\|] (Gate\|test\|build\|smoke) [\|]'; grep -cE '^[\|] (test\|build\|smoke) [\|].*[\|] [0-9]+ to [0-9]+ s( warm)? [\|]' .sdlc/adapter.md; sh .sdlc/checks/baseline-agrees-check.sh \| grep -c '^ok    time'` | `0`, `3`, `3` | at d46ae48a: `0`, `3`, `3` (the diff against `20298cc` has four deleted lines, all gate-table rows, so the first count is what confines U3). One timing changed to `99.9` in the clone prints `2` on the third line (row 1's `STALE time test`); a reworded §4 row prints `1` on the first (U1-6's measured control) |
| 8 | the nine runs are recorded with load before and after, none taken on a loaded host, the test runs print the new count, and the resolved toolchain is on record | `HF=.sdlc/handoffs/records-refresh-U3.md; grep -cE '^[\|] [1-9] [\|] .npm (test\|run build\|run smoke). [\|]' $HF; grep -E '^[\|] [1-9] [\|]' $HF \| awk -F'[\|]' '{for(i=2;i<NF;i++) if($i ~ /^ *$/) c++} END{print c+0}'; grep -E '^[\|] [1-9] [\|] .npm' $HF \| awk -F'[\|]' '{gsub(/ /,"",$4); if ($4+0 >= 10) n++} END{print n+0}'; grep -cE '^[\|] [1-9] [\|] .npm test. [\|].*all 48 test files passed' $HF; grep -cE 'typescript@7\.\|vite@8\.3\.' $HF; grep -c 'measured at 20298cc' $HF` | `9`, `0`, `0`, `3`, `2` or more, `1` | before the unit the file is absent: `0`, `0`, `0`, `0`, `0`, `grep` errors. Measured on a two-row sample: a row whose load-before cell reads `19.20` prints `1` on the third line; a row with a blank `wall (s)` cell prints `1` on the second; a run row reading `all 47 test files passed` is not counted by the fourth |
| 9 | scope wall: nothing outside `.sdlc/` and `.gitignore` differs from `20298cc`, so the K7 and K9 reruns and every plant stayed in the clone | `git diff --name-only $(git merge-base origin/main HEAD) \| grep -vcE '^\.sdlc/\|^\.gitignore$'; git status --short \| wc -l` | `0`, `0` | at d46ae48a: `0`, `0`. `echo "// probe" >> src/engine/motion.mjs` in the clone and rerun: `1` (P5's control); a `test/engine/zzz.mjs` plant left tracked in the worktree prints `1` the same way |

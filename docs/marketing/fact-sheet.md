...
| Default palettes | **16** — 8 brand families (Neutral · Primary · Secondary · Tertiary · Info · Success · Warning · Danger) + 8 data palettes (Data 1–Data 8), each carrying the same 53 roles | `role-table.json` `defaults` |
| Tonal ramp | **050–950** (19 display stops; 25 in exports with the extra stops) | `src/engine/tonal.js` `STOPS`/`EXPORT_STOPS` |
| Prime swatches per palette | **7** raw identity swatches — `brightest · brighter · bright · prime · dim · dimmer · dimmest` — derived on a **CIE L\*** lightness ladder around the palette's own key colour (its source anchor, verbatim, when the palette carries one), not on the tonal ramp. **Primitives tier, not semantic roles**: the 53-role count is unchanged and no role aliases a prime swatch. **Mode-independent** — one set, the same values in Light and Dark. Emitted in CSS · OKLCH · JSON · DTCG · Figma variables · Figma UI3 · Tailwind · Panda CSS (all 7, with `prime` as the group's default), and in the design-system export's `tokens.json`; in **Radix** only the single `prime` swatch per palette (Park UI's own scale has no slot for the other six); **not** in shadcn (its token contract is a fixed curated set). In Figma they are their own **"Color Prime"** variable collection (single `Base` mode) — **variables only, never paint styles**. Over MCP: the `get_prime` tool + a `brand://palette/{slug}/prime` resource. Shaped by the **Base chroma** and **Prime chroma** sliders (global, with a per-palette override). The ladder moved from OKHSL to CIE L\* in the same release (#681) that moved the ramps: an anchored palette's ramp now passes through its source colour at stop 500, so ramp and role values differ from the release before it | `src/engine/prime.mjs` `PRIME_STEPS`, `src/engine/collections.js` `colorPrime`, `mcp/brand-kit-core.mjs` |
...
| Export formats | CSS custom properties (HEX or OKLCH) · Tailwind v4 · shadcn/ui · **Panda CSS** · **Radix** · DTCG · JSON · Figma variables · Figma UI3 (Material) · Design System export (`tokens.json` + `DESIGN.md`, three AI-design-tool targets) · re-importable Config · Download-all `.zip`. **Panda CSS** and **Radix** each emit an ESM preset module (`panda/*.preset.mjs`, `radix/*.preset.mjs` in Download-all) that installs last in the consumer's `presets` array; Radix's colours land in Park UI's own 1–12 / a1–a12 appearance-group shape. Both are Pro-suite formats, like DTCG · Tailwind · shadcn/ui | `src/engine/exports.js` (Design System export: `src/engine/ds-export.js`; Pro-suite set: `src/ui/app-helpers.mjs` `PRO_EXPORT_FORMATS`) |
...
| Figma styles | On "Apply → Figma", **style swatches bound to the variables** — opt-out via the **"Styles"** toggle in the export drawer (default on). A **paint style per semantic role per palette family** (`Primary/onPrimary`), grouped into `scrims/` + `surfaces/` sub-folders, each bound to its Color Roles variable so it tracks **Light + Dark** automatically — **848** on the default 16-palette kit (53 × 16). **Text styles per type voice × step** (`Display/lg`, largest→smallest in the panel), bound to the Geometry + Type Primitives variables, with literal leading/tracking. **Body + Label additionally get a `/single` sibling per step/weight** (`Body/md/single`) — a 1.0-leading, single-line variant alongside the normal reading style. Plugin-created styles are updated/pruned on re-apply; the user's own styles are never touched. Download-all adds `figma/styles.plan.json` (a machine-readable plan) | `figma/binder/style-plan.mjs`, `figma/plugin/code.js` |

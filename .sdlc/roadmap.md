...
inputs: gh issue list --state open (10 issues), gh pr list --state open (1 PR), git worktree list, .sdlc/board.md, .sdlc/debt.md, plan/gate-split (#713, approved, local only), plan/preset-intent-fidelity (#681, local only), .sdlc/plans/records-followup.md, ticket #709
---
...
Count: P0 0 · P1 4 · P2 2 · P3 2 · unranked 2 (no priority label yet) · total 10.
...
| 3 | #709 | `records-followup`, `.sdlc/plans/records-followup.md`, nine of ten units 🟢, U5 (this unit) in progress, closing the roadmap now | records-refresh follow-up: nine pre-land findings and one falsified quotation landed on main with #708 | chore | P1 | L | docs | sdlc | in progress (d): PR 1 landed as #716 (squash `e9850935`); this row's own regeneration is U5, PR 2 | nothing | this cycle |
| 4 | #713 | `gate-split`, `.sdlc/plans/gate-split.md` on `plan/gate-split` (local only, not on main), status approved | `npm test` takes about 296 s after #681: split the full corpus sweeps into CI gate scripts, keep seeded samples in `npm test` | chore | P1 | L | tooling | sdlc | approved, gated (d): plan frontmatter gate G0 refuses to start any unit before #681 lands on `origin/main` | #681 landing | after #681 lands |
...
| Order | Item | Why |
|---|---|---|
| 1 | `records-followup` U5, #709 (this unit) | ten of ten units 🟢; this regeneration and its PR are all that is left |
...
| 3 | `gate-split`, #713 | approved, sized, gated on #681 landing by its own plan frontmatter; nothing else is ready to start before then |

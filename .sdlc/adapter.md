...
| ramp-identity | `node scripts/report-preset-fidelity.mjs --identity-control --base $(git merge-base origin/main HEAD)` | git; no `node_modules`, no browser | exit 0 and the last line reads `0 differing cells`. On a plan that declares a ramp movement, copy the six `identity <mode>[...]` lines into the verdict and compare them against the declaration rather than treating any nonzero reading as a fail | real 14.63 s loud (load 3.54 before, 3.19 after), stripped (the default, unanchored-path leg); real 50.38 s loud (load 3.18 before, 3.36 after) with `--authored` added (#715 U2 pass 2, this seat's own timed runs) | builder and verifier of any unit touching `src/engine/tonal.js`'s `paletteStops` or `src/ui/model.mjs`'s `rampChromaOf`/`EXPORT_STOPS`, the render functions this gate actually diffs head against base, adding `--authored` when the unit touches the anchored construction; pre-land always. Out of this gate's reach (Conductor ruling, F3, #715): `src/ui/categories/*.js`'s preset data, the default kit's palette list inside `src/ui/model.mjs`'s `defaultDocument()`, and all of `src/ui/persist.js` (`hydrate()` is called only on the base tree's own copy here, never the head's): the identity control renders the BASE tree's inputs on both engines, so a head-only change to any of these three is invisible to it. No other gate catches a head-only change to them as an identity check either: `test/engine/anchor.mjs`'s SAMPLED and FULL sweeps check self-referential invariants (monotone, window, gap, distinct, notch) that hold under any hue value, confirmed by shifting every hue in `brands.js` by +40 and running both legs: both PASS, exit 0 |

...
| U9-6 | baseline's pointer sentence credits #681 U9, not #718, for the label | 🟢 | rework 2 (F5/R2-4): `grep -n 'labelled figure' .sdlc/baseline.md` prints `211:labelled figure to check against, per PR #729's out-of-scope note (#718 made the check read`, verbatim, no join mark needed; `node .sdlc/checks/ceiling-counts-check.mjs` stays `ceiling-counts: clean` | `git show $UB:.sdlc/baseline.md \| grep -c '#718 is ruled to give this script'` prints `1`; the same grep at unit head prints `0` |
...
| F4 | a label in another gate's time cell would be graded against the `npm test` ceiling and reported under that gate's name | my call: scoped the label read to `gate === "test"` only, so a label elsewhere is neither graded nor misreported | planted `ceiling 280 to 550 s` in the `build` row's time cell: no `ceiling build` line prints, `ok    time build: baseline 1 to 3 s, adapter 1 to 3 s` is unaffected, no wrong label reported. Round 2 review (R2-1) found this claim's second half false: `cell.replace(/ceiling \d+ to \d+ s/g, "")` still stripped labels from every gate's cell, so a labelled range in a non-test cell was silently dropped from the plain-range read too, a real silent pass. Fixed in Rework 2 below (R2-1) |
...
| F6 | four handoff record defects: (a) U9-P4 wrongly said the handoff was untouched, (b) the quoted `npm test` log predated the commit by about ten hours and its own file count (14) did not match the actual diff (15), (c) "Two files" named four, (d) the U9-P2/P5/U9-8 controls were marked "not re-run" | fixed: U9-P4's row now lists `.sdlc/handoffs/pif-u9.md` in the "exactly" set and says so; a fresh `npm test` ran on the rework 1 working edits (not yet committed at that point) and is quoted below (R2-3: the run's own `08:16:33` to `08:21:23` timestamps predate the `09:22:38` commit, so "committed tree" was wrong; the F1/F2/F3/F5 fixes and the log's tree were byte-identical to what landed in `1dd3a106`, so the reading still holds for that commit); "Two files" reads "Four files"; U9-P2, U9-P5 and U9-8 each carry a control run this pass (rows above) | each correction is the row it corrects, above; nothing else in the handoff was rewritten |
...
## Rework 2 (review `.sdlc/verdicts/pif-u9-review.md`, Round 2, FIX-FIRST)

Dispatch scope: R2-1, R2-3, R2-4. R2-2 (plan row U9-3's expected output is stale against rework
1's F2 fix) is the Orchestrator's to amend in a plan revision, not touched here. R2-5 (run every
file-editing control in a scratch clone, never the worktree) is followed throughout this pass.

| # | Finding | Fix | Control |
|---|---|---|---|
| R2-1 | `cell.replace(/ceiling \d+ to \d+ s/g, "")` stripped a `ceiling N to M s` label from every gate's cell, not just `test`'s, so a labelled range planted in another gate's time cell vanished from the plain-range read too and passed silently. Rework 1's F4 row claimed "no wrong label, no silent pass either"; the second half was false | scoped the strip to the `test` cell: `const plain = gate === "test" ? cell.replace(/ceiling \d+ to \d+ s/g, "") : cell;` (`.sdlc/checks/baseline-agrees-check.sh:32`). The F4 row above is corrected to say so | scratch clone (`git clone -q --shared`, F1/F2/F3/F5/R2-1 applied via the uncommitted diff), `1 to 3 s warm` changed to `1 to 3 s warm; ceiling 1 to 2 s` in the `build` cell: `STALE time build: baseline 1 to 3 s, adapter 1 to 3 s, 1 to 2 s`, `stale total: 1`, exit 1. Restored, `git status --short` on that file empty. Read-only sanity at unit head (no plant) confirms `stale total: 0` still |
| R2-3 | Rework 1's F6 row said the fresh `npm test` "ran on this rework's committed tree"; the run's own `pre-run: Wed Sep 23 08:16:33` to `post-run: Wed Sep 23 08:21:23` timestamps predate the `1dd3a106` commit, made at `09:22:38`, so the run was over the rework 1 working edits, not a commit | reworded the F6 row: the run was on the working edits, and no code changed between that run and the commit (only this handoff's own text was edited afterward), so the reading still describes `1dd3a106`'s tree exactly | textual correction, no re-run needed |
| R2-4 | U9-6's own criteria row still cited `grep -n 'labelled'` and quoted three source lines joined by `...` with no `altered:` mark, so it did not confirm F5's real fix (re-wrapping `labelled figure` onto `.sdlc/baseline.md:211`) | U9-6's row now cites `grep -n 'labelled figure'` and quotes line 211 verbatim, no join | `grep -n 'labelled figure' .sdlc/baseline.md` prints `211:labelled figure to check against, per PR #729's out-of-scope note (#718 made the check read` |

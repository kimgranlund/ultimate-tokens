...
| `npm run build` | 3/3 | 0 | 3.06 · 1.34 · 1.36 | `wrote figma/plugin/ui.html 4122.4 KB` `re-measured after hex-oklch-dedupe U1's deletion (build confirmed, exit 0, same 4122.4 KB); see the #681 U7, #681 U10, #739 U1, #738 U1, #731 U1 and rule-gates U5 corrections below for the figure's history; seconds carry forward from the prior reading, since the deletion is small and the build's own timing is not re-clocked; rule-gates U5's own sweep and this merge move the figure again, see the newest Correction below` |

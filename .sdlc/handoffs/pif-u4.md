...
| U6 prime ladder | `31939051` | `src/engine/prime.mjs`'s 3-way merge left a non-functional hybrid: U1's old OKHSL widening-loop (referencing variables that no longer existed) mixed with U6's real construction. Rewrote `primeSwatches` from scratch combining U1's anchor detection with U6's equal-compress ladder. 9 of `test/engine/prime.mjs`'s own 21 gates assumed the non-anchored cusp construction the standalone U6 branch was built against; re-derived each against the real, landed anchors. All 21 gates pass on the integrated tree (detail in the commit body). |
...
**Negative control that reds:** the gate's own negative-control block (unchanged) copies
`src/engine/tonal.js`'s source into a scratch string and deletes one call site's text, re-runs the grep,
and asserts the count drops below 5 — proving the assertion is not vacuously satisfied by, say, matching
the word "chromaEnvelope" inside a comment. This control still reds correctly against the modified
source (verified as part of `node test/engine/tonal.mjs`'s own run — the `ac004-greps`/C7 group passes).
...
- `test/engine/anchor.mjs` — 3 gate groups failing (§7.4-7.6, one of which, notch, is the Q-C
  pending-U4 re-measurement this unit was explicitly asked to produce, not fix)
...
### 7.4-7.5 anchor.mjs `anchor-ladder` (order/dupe-allow-list) and `prime-identity-control` — two distinct construction findings, neither fixed

**Finding A — the F1-widening-search gap for out-of-window anchors (21 corpus sources).** U1's original
anchored-ladder design used an "F1 widening search" specifically so a source whose anchor sits at or
past `[PRIME_L_MIN, PRIME_L_MAX]` (~[12.25, 96.88] L*) still gets a real six-rung ladder — by letting
`prime` itself render OUTSIDE the window while the six other rungs spread normally inside it (this is
what `ORDER_ALLOW`/`DUPE_ALLOW` are for: named exceptions where `prime` does NOT sit strictly between
its neighbours). U6's equal-compress rebuild replaces that mechanism entirely (both sides of the ladder
take the SAME step, `min(STEP_L, roomUp, roomDown)`), and my merge-conflict resolution (§1, `prime.mjs`
was a non-functional broken hybrid before the rewrite) did not re-add an F1-style widening step, since
doing so would be inventing new construction, not integrating existing verified work.
...
also FAILs directly (invariant (a), documented as "0 exceptions... unconditionally" — no allow-list): the
collapse breaks even the SIX-rung-only monotone check, not just the prime-placement one.
...
**This needs an owner/engine-level ruling, not a test re-pin**: either port an F1-style widening step
onto the equal-compress construction for out-of-window anchors, or rule that a flat six-rung collapse
(0.62% of the anchored corpus) is acceptable and re-freeze `ORDER_ALLOW`/`DUPE_ALLOW`/the `sixMono`
invariant's own wording to match. I have not touched `test/engine/anchor.mjs` for this.
...
### 7.6 anchor.mjs `anchor-ramp lone-spike` — 64 unresolved, not triaged to root cause

`64` near-achromatic spike artifacts in even mode (both neighbours' OKLCH C <= 0.05, the stop's own C
exceeds both by > 0.03), no allow-list, described by the gate itself as "a real hit is the bug this pass
exists to prevent." Not root-caused this pass — time did not allow tracing each of the 64 to a specific
mechanism the way §7.4-7.5 were traced. Reporting the count as measured; recommend a follow-up unit or a
...
**Max |dL\*| / max dChroma per stop/mode** (corpus, export25 stop set, vs bf2aaf6, n=94,500 stop-cells
per mode):

| mode | max \|dL*\| | at | max dChroma | at |
|---|---|---|---|---|
| perceptual | 19.40 | film "TRON: Legacy" secondary, stop 500 | 32.25 | brands "Burger King" primary, stop 500 |
| even | 12.45 | film "2001: A Space Odyssey" tertiary-muted, stop 500 | -43.75 | brands "Nike" primary-muted, stop 600 |
| peak | 50.75 | cuisine "Cocktails" tertiary-muted, stop 650 | 43.85 | brands "Burger King" primary, stop 500 |
...
design, not a defect. Peak mode's larger, non-stop-500 movements (worst at stop 650) trace to U3's F1
peak-cap trade (§7.3).

**Full per-mode median/p90 table, both stop sets, all stops** (corpus, |dL*| vs bf2aaf6):

| mode | stop set | n | median \|dL*\| | p90 \|dL*\| |
|---|---|---|---|---|
| perceptual | display19 (19 stops) | 71,820 | 2.9610 | 6.7377 |
| perceptual | export25 (25 stops) | 94,500 | 2.2325 | 6.3673 |
| even | display19 | 71,820 | 2.0441 | 6.1827 |
| even | export25 | 94,500 | 1.3458 | 5.8177 |
| peak | display19 | 71,820 | 6.0431 | 26.1404 |
| peak | export25 | 94,500 | 4.8919 | 23.2880 |
...
**C4's identity-control line:** red — see §7.5, Finding B. Not a regression; the reference construction
predates U6's own sibling-unit scope change and needs reconciling, not silently loosening.
...
**#668's count:** see the dedicated §10 section — inconclusive, not closed this pass.
...
| Non-anchored: unchanged (e, h) | primeSwatches unchanged for non-anchored palettes | **Miss** — U6's ladder rebuild changes the non-anchored construction too (§7.5, Finding B); the C4 reference implementation was never updated to account for this sibling-unit scope | **Miss, flagged** |
...
| #668: 0 upticks at stop 800/mirror, all three modes, both stop sets | | Measured 0/0/0/0/0/0 at integration head — **but the negative control against bf2aaf6 also reads 0, not the stated 11/46/11/43 baseline** (§10) | **Inconclusive, flagged** — cannot confirm closure without a working negative control |
...
Three misses/flags total: C4's non-anchored identity control (a real, unreconciled cross-unit scope gap,
§7.5), #668's inconclusive negative control (§10), and C7's count (already explained as a legitimate
reading change, §2, not a fresh miss). All three are findings, not failures hidden from this report.
...
### #668 — the measured CIELAB L* rise at stop 800 and its light-side mirror: INCONCLUSIVE, not closed

Swept the full corpus (3,780 palettes) + 16 default-kit palettes, all three modes, both stop sets (19
display / 25 export), measuring adjacent-stop pixel L* (`lstarFromRgb`) for any rise anywhere in the
ramp (a superset of "at stop 800 specifically," so it would catch the named defect wherever it lands).

**Integration head: 0 rises, every mode, every stop set** (`perceptual|display19` through
`peak|export25`, all six combinations read 0). This matches what `test/engine/categories.mjs`'s own
`ramp-monotone` comment already records (a corpus-wide sweep on the U1+U2+U3 tree measured 0 rise-cells
everywhere), now reconfirmed on the U1+U2+U3+U6 tree.

**Negative control (bf2aaf6, same probe): also 0 rises, every combination.** This does NOT reproduce
the addendum's stated baseline (11 perceptual / 46 peak on the 25-stop export ramp, 11/43 on the 19-stop
display ramp). I directly tested two of the eleven named witnesses
(`test/engine/categories.mjs`'s `WITNESSES` array — film "Touch of Evil" secondary, lift -40; music
"Black metal" secondary, lift -40) against bf2aaf6's own engine and own category files, in all three
modes: **no rise in any mode for either witness.** `bf2aaf6`'s `tonal.js` already carries `liftStop`
and the `#648`-bound damping-position fix (confirmed by grep), so whatever construction state actually
produced the cited 11/46/11/43 figures does not appear to be reproducible at the commit I resolved as
`bf2aaf6` (`bf2aaf65`, "test(contrast): flat AA floor over the curated corpus...").

**Per the addendum's own standard** ("A control that does not red means your probe is vacuous, and a
vacuous probe is worse than no measurement" / "If either still reproduces, say exactly that... Do not
soften a reproduction into a caveat"): I am not claiming #668 closed. The integration-head measurement
(0/0/0/0/0/0) is real and consistent with the codebase's own existing analysis, but without a working
negative control I cannot certify it as proof of closure versus a probe that was never capable of
finding the defect in the first place. **This needs the exact reproduction command and/or commit that
produced the 11/46/11/43 figures**, so the negative control can be re-pointed at a state that is known
to exhibit them. Flagging to the owner rather than asserting either "closed" or "still open."
...
If both addendum measurements were green this line would read "Closes #681, closes #686, closes #668."
Given #668's negative control does not reproduce (§10), the PR body should read **"Closes #681, closes
#686"** only, with #668 left open pending the reproduction-command clarification above.

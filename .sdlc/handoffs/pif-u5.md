...
| `.sdlc/adapter.md` (second edit) | §1 gains a `ceiling-counts` gate row, so something names the check. Declared in §10 beside the budget-note hunk; line 192 is untouched by both |
| `.sdlc/checks/ceiling-counts-check.mjs` | New. Re-derives the ceiling section's counts and its graded/explicit/unsupportable partition from the reading rows and checks the prose and the adapter's pointer against them. Written because the same section shipped three count defects in three passes, the last of which no total could see. Parse scoped to the ceiling section so an unrelated table added to `baseline.md` cannot red it. Negative controls constructed, see §8 |
...
| `node .sdlc/checks/ceiling-counts-check.mjs` | **exit 0**, `ceiling-counts: clean`, eleven assertions, and it is named in `.sdlc/adapter.md` §1's gate list so something calls it rather than it sitting uninvoked. It is the first gate added on this plan that arrived with a WORKING negative control, which matters because five of the six vacuous gates this plan found read green while measuring something other than what they named. Three independent demonstrations that it can fail: this builder's own, restoring the word `twelve`; the lane lead's, reproduced on a copy outside this worktree; and the verifier's **nine mutations, each exit 1**. A fourth control runs the other way, that it does not fail for the wrong reason: appending an unrelated reading table under a later heading leaves it `clean`, because the parse is scoped to the ceiling section. It earned its place on first use. Recording run 11 added a reading, and the ad-hoc recount this builder had been running updated the totals but not the partition sentence. The check failed with `FAIL  prose 'other N' == total - graded  (prose 16, measured 17)` and `FAIL  prose explicit count == measured  (prose 13, measured 14)`, which is the exact defect class the verdict raised, caught in seconds by a script instead of in a review round by a person |
...
Run 11 retired it for one commit: a full `npm test` over that tree,
taken once `ps` showed the host clear. The provenance pass that follows it moved the tree again, so
the argument is load-bearing once more and is restated rather than quietly reused. The delta from
run 11's tree to this head is: three reading rows gaining tree shas, one gate log committed and one
relativized, the counts check scoped to its section, a gate row in `.sdlc/adapter.md` §1, and this
prose. No file outside `.sdlc/` is touched, which `git diff --name-only 8f037dd2 HEAD` confirms by
printing `.sdlc` alone.

Every check that can observe that delta was run on it and is green, including two the earlier
passes did not have: `ceiling-counts-check.mjs`, which reads both edited files, and
`baseline-agrees-check.sh`, which parses the adapter table the new gate row sits in. A full run was
not taken at this head because `ps` showed two `npm test` runs, two gate scripts and three
`test/run.mjs` processes live on a host at load 89.66. One will be taken when the host clears and
the result reported, whether or not the Verifier has moved by then.
...
and it prints TWO hunks, both declared. The first is that note. The second is a gate row added to
§1 for `.sdlc/checks/ceiling-counts-check.mjs`, because review pass 4 (Y4) found that nothing
invoked the check, and a check that can fail but is never called is the same class of defect as one
that cannot fail, of which this plan has already found six. Line 192 is untouched, which is the thing the invariant

...
| `npm run build` | 3/3 | 0 | 3.06 · 1.34 · 1.36 | `wrote figma/plugin/ui.html 4130.1 KB` `re-measured after chroma-floor U1's shoulder term (build confirmed, exit 0, same 4130.1 KB); see the #681 U7, #681 U10, #739 U1, #738 U1 and #701 U1 corrections below for the figure's history; seconds carry forward from the prior reading, since this unit's own build timing was not re-measured under this correction` |
...
| `npm run gate:mode-isolation` | 0/3 | 0 | 20 · 27 · 34 | `pass  mode-isolation: ... match fixture` `new row, chroma-floor #701 U1; NOT quiet-host (load 12.33 / 10.44 / 23.70 at start, all above the 5-load bound; the host was busy with concurrent agent runs across this whole session), so this is 0 counted readings, not 3/3 - a genuine quiet-host set is owed before pre-land` |
| `npm run gate:even-dips` | 0/3 | 0 | 28.71 · 26.37 · 32.82 | `PASS` `new row, chroma-floor #701 U2; NOT quiet-host (1-minute load 100.26 / 96.32 / 66.76 at start and 96.32 / 66.76 / 59.97 at end, all far above the core count; concurrent agent runs), so this is 0 counted readings, not 3/3; a genuine quiet-host set is owed before pre-land` |
...
Correction (2026-09-26, plan chroma-floor U1, #701): the ui.html figure moves again, from 4125.3 KB to 4130.1 KB. The cause is named: U1 adds `EVEN_NEIGHBOURHOOD_R` and the neighbourhood-plateau shoulder term to `chromaEnvelope` in `src/engine/tonal.js`, which is inlined into `figma/plugin/ui.html`. The figure is program output: `npm test` and `npm run build` both printed `wrote figma/plugin/ui.html 4130.1 KB` in `.worktrees/cf-U1`, reproduced across multiple runs, and measuring the committed file the way `baseline-agrees-check.sh` measures it gives the same 4130.1. Only the KB cell moves, on the #681 U7/U10 precedent: the unit whose change made the figure stale repairs it in its own commit.

...
export class DrawerMixinImpl {
  ...
  renderDrawer(view) {
    ...
    const FORMAT_GROUPS = [
      ["Colors", [["css", "Hex"], ["oklch", "OKLCH"], ["tailwind", "Tailwind v4"], ["shadcn", "shadcn/ui"], ["panda", "Panda CSS"], ["figma", "Figma"], ["ui3", "Figma UI3"], ["dtcg", "DTCG"], ["json", "JSON"]]],
    ...
    const PRO_LABEL = { dtcg: "DTCG", tailwind: "Tailwind v4", shadcn: "shadcn/ui", panda: "Panda CSS" };
  ...
  _zipReadme(s, sys) {
    ...
    if (sys.color) {
      rows.push(
        "| `css-hex/` · `css-oklch/` | The palette + 53 semantic roles per palette, plus fixed system constants (`--dialog-backdrop`, `--white`, `--black`), as CSS custom properties (two co-equal color formats) |",

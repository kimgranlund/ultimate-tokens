...
| Prime swatches per palette | **7** raw identity swatches — `brightest · brighter · bright · prime · dim · dimmer · dimmest` — derived on the palette's own key colour's OKHSL lightness ladder, not on the tonal ramp. **Primitives tier, not semantic roles**: the 53-role count is unchanged and no role aliases a prime swatch. **Mode-independent** — one set, the same values in Light and Dark. Emitted in CSS · OKLCH · JSON · DTCG · Figma variables · Figma UI3 · Tailwind · Panda CSS (all 7, with `prime` as the group's default), and in the design-system export's `tokens.json`; in **Park UI** only the single `prime` swatch per palette (Park's own scale has no slot for the other six); **not** in shadcn (its token contract is a fixed curated set). In Figma they are their own **"Color Prime"** variable collection (single `Base` mode) — **variables only, never paint styles**. Over MCP: the `get_prime` tool + a `brand://palette/{slug}/prime` resource. Shaped by the **Base chroma** and **Prime chroma** sliders (global, with a per-palette override); at the shipped defaults (both **100**) every ramp and role value is unchanged from the previous release | `src/engine/prime.mjs` `PRIME_STEPS`, `src/engine/collections.js` `colorPrime`, `mcp/brand-kit-core.mjs` |
...
| Export formats | CSS custom properties (HEX or OKLCH) · Tailwind v4 · shadcn/ui · **Panda CSS** · **Park UI** · DTCG · JSON · Figma variables · Figma UI3 (Material) · Design System export (`tokens.json` + `DESIGN.md`, three AI-design-tool targets) · re-importable Config · Download-all `.zip`. **Panda CSS** and **Park UI** each emit an ESM preset module (`panda/*.preset.mjs`, `park-ui/*.preset.mjs` in Download-all) that installs last in the consumer's `presets` array; Park UI's colours land in Park's own 1–12 / a1–a12 appearance-group shape. Both are Pro-suite formats, like DTCG · Tailwind · shadcn/ui | `src/engine/exports.js` (Design System export: `src/engine/ds-export.js`; Pro-suite set: `src/ui/app-helpers.mjs` `PRO_EXPORT_FORMATS`) |

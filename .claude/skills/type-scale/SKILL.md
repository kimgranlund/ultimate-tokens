...
- **Leading constants** — FIXED per-role, uniform across treatments: display **0.8** (< 1, large type sets tight) · heading-family (Headline/Sub-heading/Title) **1.125** · prose (Body/Lead/Sub-title/Tiny/Label/Label-mono/Kicker + the interactive UI voices) **1.4–1.5** (Body-mono 1.5). The 1.0 single-line fit lives in `singleLineHeight` on the BOX voices (Kicker/UI-control/UI-widget), not in a voice's flow leading. Retune a per-voice `*Lead` knob only for a deliberate character exception → foundations §3.

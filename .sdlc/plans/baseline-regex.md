...
| P4 | branding gate clean | block P4 | `branding: clean (N files scanned)` (N not pinned; measured 589 today), `exit 0` | in the clone: `cp docs/reference/references/decision-records.md docs/x.md`, rerun: `FAIL: 3 branding violation(s) across 519 files`, `exit 1` (measured today; the records exemption is by path) |

...
5. **This handoff commit could trip the branch hook's `verdict.py` check (block 2 of the future `unit/df-ids` pre-commit, staged `.sdlc/handoffs/*.md`).** Two checks, no bypass. First, the currently *installed* hook (`core.hooksPath` -> the plugin's `0.1.0` copy) has no block 2 at all, so my real handoff commit (`81347bb`) ran clean: `exit 0`, no refusal. Second, since the concern is about the *future* hook landing mid-plan, I simulated it: cloned the tree at the rename commit (`e22f8ba`), pointed `core.hooksPath` at the extracted `unit/df-ids` `githooks/` (which does carry block 2's `verdict.py check`), staged the same handoff content, and committed: `commit 0`, no refusal. So the handoff as written already passes `verdict.py check` on its own merits (it has no rows the check would read as new-and-failing); no edit, disable, or bypass was needed either way. Had the simulation refused it, the fix would have been to correct the handoff's own evidence/control cells, never to skip the hook.

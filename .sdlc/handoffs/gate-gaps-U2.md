...
| P2-F1 (re-review: "run by" cell over-narrowed) | Revision 7 (F3) removed the input paths only, not step 6's `src/engine/`/`src/ui/model.mjs` triggers; my first fix narrowed the cell to `paletteStops`/`rampChromaOf`/`EXPORT_STOPS` by name, which drops real triggers this gate does catch. Reviewer proved the gap: `rampChromaOf` multiplied by `0.9` in a mutated `resolve.mjs` reads 896 differing cells, not the false green the narrowed cell implied. Restored the "run by" cell to `src/engine/` and `src/ui/model.mjs`, with `--authored`/pre-land wording as before; the out-of-reach sentence for categories/, the kit, and `persist.js` stays. | this commit |

...
try {
  ...
  // AC-006/EX-8: the Panda CSS + Radix drawer tabs render real code in a real browser.
  ...
  await evalJS(`${el}.exportTab="radix";${el}.render()`); await sleep(200);
  ok(await evalJS(`(()=>{const t=${el}.querySelector(".drawer-pre");return !!t && /^\\/\\* ultimate-tokens export schema \\d+ \\*\\/\\n\\/\\* Radix preset, generated by Ultimate Tokens\\./.test(t.textContent)})()`), "Radix export tab renders code starting with the schema stamp then the header comment");
  const radixShot = await send("Page.captureScreenshot", { format: "png" });
  writeFileSync(resolve(OUT, "export-radix.png"), Buffer.from(radixShot.data, "base64"));
  console.log("  · screenshot → smoke-out/export-radix.png");

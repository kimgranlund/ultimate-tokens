...
| `npm run build` | 3/3 | 0 | 3.06 · 1.34 · 1.36 | `wrote figma/plugin/ui.html 4125.5 KB` `re-measured after docs-repair U3's header rewording (build confirmed, exit 0, same 4125.5 KB); see the #681 U7, #681 U10, #739 U1, #738 U1 and #751 U3 corrections below for the figure's history; seconds carry forward from the prior reading, since the header changes only comment text, not the build's own timing` |
...
Correction (2026-09-25, plan docs-repair U3, #751): the ui.html figure moves again, from 4125.3 KB to 4125.5 KB. The cause is named: U3 rewords the file headers of `src/ui/app.js` (names Ultimate Tokens, not the retired maker brand), `src/ui/persist.js` (a new first-comment line stating the file's purpose), and `src/engine/geometry.mjs` (drops the private `.claude/docs/` path from its source note), all three inlined into `figma/plugin/ui.html`. The figure is program output: `npm run build` printed `wrote figma/plugin/ui.html 4125.5 KB` in `.worktrees/dr-U3`, reproduced on a second run, and measuring the committed file the way `baseline-agrees-check.sh` measures it gives the same 4125.5. Only the KB cell moves, on the #681 U7/U10 precedent: the unit whose change made the figure stale repairs it in its own commit.

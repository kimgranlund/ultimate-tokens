...
- Amendment paragraph: byte-identical to plan line 129 with `<date>`→`2026-09-20`; placed after §2.1 item 4 + its blank line, before `### 2.2`, one paragraph + one trailing blank line. (An unrelated pre-existing 2026-09-18 amendment sits earlier in the file, and I initially mis-grabbed it with a loose awk pattern in scratch work, then anchored on the exact date and confirmed the right one; no effect on the finding.)

...
|---|---|---|---|
| D1 | `sweep-scan`/`sweep-delete` failures never reply → `sweepBusy` wedges; Cleanup panel disabled for the session | 04 §B; the `postMessage` catch at figma/plugin/code.js:253-255 special-cased only `apply` (fixed in #459) | code.js catch posts `sweep-scanned {texts:[],paints:[]}` / `sweep-done {removed:0}`, mirroring the apply carve-out |
...
- `_okL` (`src/engine/tonal.js:350`) module-level memo Map is the one true exception to "engines are pure, no module state" — bounded and harmless; worth one comment acknowledging it (04 §B).

...
| F7 | Regroup safety | [review] | (app path) `rebuildSemantic` re-creates `Color Roles` only, leaves Color Primitives intact, doesn't duplicate the collection, and stays behind the ALWAYS-warn gate (`renderApplyGate` renders the DON'T-SHOW-AGAIN checkbox as `rebuild ? false : checkbox`; `confirmApplyGate` persists consent only when `!rebuild`). Since #629 the gate carries a second, UNCONDITIONAL "Published library" checkbox that renders on both paths: judge the don't-show-again one, not the count | 1: Regroup cookieable, or it touches Color Primitives, or duplicates the collection · 3: gated + isolated · 5: + the canonical regroup order (scrims last 7) preserved |

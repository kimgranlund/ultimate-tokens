...
|---|---|---|---|
| `load-config` | `app.js:1007` (probe), `app.js:2324` (explicit load), posting `type: "load-config"` | `code.js:225-228` (`msg.type === "load-config"`) | `config-loaded` |
| `read-variables` | `app.js:1008` (probe), `app.js:2378` (manual re-read), posting `type: "read-variables"` | `code.js:236-239` (`msg.type === "read-variables"`) | `variables-read` |
| `load-sets` | `app.js:1009` (probe), posting `type: "load-sets"` | `code.js:243-246` (`msg.type === "load-sets"`) | `sets-loaded` |
| `read-float-variables` | `apply-gate.js:45` (gate open), posting `type: "read-float-variables"` | `code.js:240-242` (`msg.type === "read-float-variables"`) | `float-variables-read` |
| `list-fonts` | `typography.js:808` (one-shot), posting `type: "list-fonts"` | `code.js:229-235` (`msg.type === "list-fonts"`) | `fonts-listed` |
| `save-sets` | `app.js:1158` (`persistSets`), posting `type: "save-sets"` | `code.js:247-249` (`msg.type === "save-sets"`) | **none** (fire-and-forget) |
| `save-config` | `app.js:2311`, posting `type: "save-config"` | `code.js:222-224` (`msg.type === "save-config"`) | **none** (only a `figma.notify`, not a postMessage) |
| `apply` | `apply-gate.js:106`, posting `type: "apply"` | `code.js:133-221` (`msg.type === "apply"`) | `apply-done` or `apply-error` |
| `sweep-scan` | `apply-gate.js:221`, posting `type: "sweep-scan"` | `code.js:250-256` (`msg.type === "sweep-scan"`) | `sweep-scanned` |
| `sweep-delete` | `apply-gate.js:248`, posting `type: "sweep-delete"` | `code.js:257-266` (`msg.type === "sweep-delete"`) | `sweep-done` |
...
| `config-loaded` | `code.js:227` (`type: "config-loaded"`) | `gen-figma-ui.mjs:34` | `app.js:2338 applyLoadedConfig` | `this.fileConfig` or opens a new set | yes, both branches |
| `variables-read` | `code.js:238` (`type: "variables-read"`) | `gen-figma-ui.mjs:36` | `app.js:2382 receiveLiveVariables` | `this.liveVars`, `this.liveVarsFound` | yes |
| `float-variables-read` | `code.js:242` (`type: "float-variables-read"`) | `gen-figma-ui.mjs:39` | `apply-gate.js:299 receiveLiveFloatVariables` | `this._liveFloatVars` | yes |
| `sets-loaded` | `code.js:246` (`type: "sets-loaded"`) | `gen-figma-ui.mjs:42` | `app.js:1166 receiveStoredSets` | `this.sets` (guarded) | yes |
| `fonts-listed` | `code.js:235` (`type: "fonts-listed"`) | `gen-figma-ui.mjs:45` | `typography.js:811 receiveFigmaFonts` | `this._figmaFonts` | yes |
| `apply-done` | `code.js:221` (`type: "apply-done"`) | `gen-figma-ui.mjs:48` | `apply-gate.js:150 onApplyDone` | `this._applyBusy=false`, `this.applyGateOpen=false` | yes |
| `apply-error` | `code.js:276` (`type: "apply-error"`) (catch-all, apply only) | `gen-figma-ui.mjs:49` | `apply-gate.js:173 onApplyError` | `this._applyBusy=false` | yes |
| `sweep-scanned` | `code.js:256` (`type: "sweep-scanned"`) | `gen-figma-ui.mjs:52` | `apply-gate.js:225 receiveSweepScan` | `this.sweepResults`, `this.sweepBusy=false` | yes |
| `sweep-done` | `code.js:265` (`type: "sweep-done"`) | `gen-figma-ui.mjs:53` | `apply-gate.js:252 onSweepDone` | `this.sweepBusy=false`, clears results | yes |
...
**Asymmetry (the one real design gap):** `apply` is the only inbound request with a guaranteed reply on failure — `code.js`'s outer `catch` (`code.js:267-281`) explicitly special-cases `msg.type === "apply"` to post `apply-error`. `sweep-scan` and `sweep-delete` get NO such carve-out [since fixed: `code.js:277-278` now post empty `sweep-scanned`/`sweep-done` from the same catch]: if `figma.getLocalTextStylesAsync()`/`sweepCandidates`/the delete loop throws, the catch only calls `figma.notify(...)` — no `sweep-scanned`/`sweep-done` is ever posted. See (B) below — this is a real wedge, not theoretical.

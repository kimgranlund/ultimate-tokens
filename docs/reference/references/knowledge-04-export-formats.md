...
- **Reference form** (`exportRadix(state, { refs: true })`, ticket #638): the same preset shape as
  ...
  resolve only once the kit's own CSS or OKLCH export is also loaded, and `exportRadixModule`'s
  header says so. `exportAll` exposes it as `radixRef` beside `radix`.

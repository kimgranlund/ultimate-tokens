...
| P1 | `npm test` green with no `node_modules`, tree byte-stable | 🟢 | `✓ all 47 test files passed`, exit 0, `git status --short` 0 lines, no `node_modules` in the worktree at start. 60.16 s, in range | in the clone: `"scrim` to `"scrimX` in `role-table.json` → 3 `FAIL` lines, `engine/semantic.mjs FAIL`, `refs-canonical — ordered key set != canonical`, `✗ 1/47 test file(s) failed` |

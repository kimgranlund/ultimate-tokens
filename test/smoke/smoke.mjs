...
try {
  ...
  // AC-006/EX-8: the Panda CSS + Park UI drawer tabs render real code in a real browser.
  await evalJS(`${el}.exportTab="panda";${el}.render()`); await sleep(200);
  ok(await evalJS(`(()=>{const t=${el}.querySelector(".drawer-pre");return !!t && /^\\/\\* ultimate-tokens export schema \\d+ \\*\\/\\n\\/\\* Panda CSS preset, generated by Ultimate Tokens\\./.test(t.textContent)})()`), "Panda CSS export tab renders code starting with the schema stamp then the header comment");
  const pandaShot = await send("Page.captureScreenshot", { format: "png" });
  writeFileSync(resolve(OUT, "export-panda.png"), Buffer.from(pandaShot.data, "base64"));
  console.log("  · screenshot → smoke-out/export-panda.png");
  await evalJS(`${el}.exportTab="parkui";${el}.render()`); await sleep(200);
  ok(await evalJS(`(()=>{const t=${el}.querySelector(".drawer-pre");return !!t && /^\\/\\* ultimate-tokens export schema \\d+ \\*\\/\\n\\/\\* Park UI preset, generated by Ultimate Tokens\\./.test(t.textContent)})()`), "Park UI export tab renders code starting with the schema stamp then the header comment");
  const parkuiShot = await send("Page.captureScreenshot", { format: "png" });
  writeFileSync(resolve(OUT, "export-parkui.png"), Buffer.from(parkuiShot.data, "base64"));
  console.log("  · screenshot → smoke-out/export-parkui.png");
  await evalJS(`${el}.exportTab="css";${el}.render()`); await sleep(80);

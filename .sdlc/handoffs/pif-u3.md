...
# U3 handoff, anchor-centred chroma envelope, all modes, grade l4 (pass 7 + review 2)
...
the first time (Q7, new), and it fails the numeric table under both readings of C6's ambiguous prose, reported honestly with real numbers, not silently made to pass. Q1, Q3 and Q6 are resolved (Q6 by
...
| C6-iii-new | "0 above 100%" (reading a), GENERATED palettes (dampAmp 0), even and peak modes (iii), plus perceptual's bounded cusp-run clause (iii-b), each with the named Adia carve-out | `test/engine/tonal.mjs` "chroma-envelope" (C6 iii / iii-b) | pass, all three modes: 0 non-Adia above-100%/over-bound instances; Adia's own instances confirmed still present in each (the carve-out isn't stale). A permanent dip gate (iv, review 2 F1, extended U3 review 3 N1) also passes: 6 peak-mode dips (4 fully natural, 2 made milder by the cap, not created by it, Q7; corrected from "6 natural," U3 review 5 S3) at or under the reviewer's 7-dip cap-off baseline, 0 perceptual dips, and 53 even-mode dips (51 at stop 350, 2 at 650, depth 3.01-4.06 C), all named pending-owner in `EVEN_DIP_BASELINE` (U3 review 3, N1: a real ramp-shape defect from `evenChroma`'s floor construction, one fix attempt tried and reverted because it regressed `even|100`/`even|900`'s median/p90; full mechanism, the fix attempt's numbers, and the recommendation are in `.sdlc/questions/pif-u3-yield.md`'s second item) | a scratch copy of a non-Adia doc with `dampAmp` forced to 70 produces an above-100%/over-bound instance NOT in `ADIA_CARVEOUT`, caught as unlisted in every mode; the dip gate's own negative control (a patched engine reproducing F1's pre-fix bug pattern, now wired through the real `findDips` per U3 review 4 R4) produces 173 dips, well past the 6 baseline; the even dip gate's own negative control (a patched engine amplifying `chromaFloor` 1.6x, same wiring fix) produces 328 dips, well past the 53 baseline; a third probe (dropping "even" out of the toneMode loop entirely) now fails loudly naming the orphaned baseline, rather than silently passing (U3 review 4 R4) |
...
every stop but one to the anchor would leave that one stop nearly double its now-flattened neighbours, a lone spike, not a graceful exemption, exactly the failure mode the brief pre-authorized stopping for.
...
`peak` are untouched (0 hex diffs, 334,048 rendered cells, both stop sets). This reshapes nearly every
...
- **U2:** review finding 8 (latent hazard): `chromaEnvelope(stop, anchorStop, lift, controls)` genuinely
  ...
  `intended500` (`paletteStops`) and `lightnessAt(500, t500)` (`okhslStops`). A third site, the `const
  c500 = evenChroma(maxc500, intended500, envelopeAt.get(ANCHOR_STOP), controls.chromaFloor)` line in
  `paletteStops` (`src/engine/tonal.js`, cited by symbol rather than a line number after U3 review 5 S1
  found this citation had already gone stale twice, most recently `:381` when the actual line was
  `:394`) on the seed-hue-solving branch, carries the same hardcoded-500 shape (U3 review 2, F7). If
  U2 threads a per-palette anchor stop, the seed's saturation will follow it while the seed's lightness
  stays at 500, at all three sites.
...
second workaround is needed, stop". U3 review 2 then separately authorized ONE retry of step 2 on top of
its own F1 fix (below), since step 2 reuses `solveLForTone`/`refineNearestRgb`; the retry closed the same
...
`.sdlc/questions/pif-u3-yield.md` carries the owner-facing detail for BOTH pending items this pass leaves
open, side by side with achieved-vs-bar figures and a recommendation: its FIRST item is the step 2
ship-vs-revert tradeoff above (revert recommended); its SECOND item is the 53 named even-mode dips
(`EVEN_DIP_BASELINE`, U3 review 3 N1) with the one fix attempt's mechanism and numbers, also recommending
revert (kept as the shipped, gated, pending-owner state).

U3 review 2 also shipped, on top of pass 7: F2 (the C6 iii gate's `above100Violators` had an early return
that skipped every generated palette, so it could never fail; removed, with a lift-scoped negative
control), F1 (the peak cap's HCT fallback undershot the anchor via an overshooting single-step exit plus
a `Math.min(chroma, target)` fallback that locked the undershoot in, and used an Abney-drifted proxy hue;
fixed via a 24-iteration bisection, a target-direct fallback, an Abney-corrected polish hue, and a
`chromaFloor` guard on the 8-bit polish; dips 221 -> 6, all confirmed natural, now gated permanently with
a named baseline and negative control), and F3 (23 stale `hpg-role-contrast` comments re-measured; the
"even is byte-identical" claim scoped to the R2 landing, before pass 7 moved even mode again; perceptual
Success light's undisclosed drop, 6.1815 -> 6.1018, recorded in Q6's obligation set). Full detail in
`.sdlc/questions/pif-u3.md`'s review-2 addenda.

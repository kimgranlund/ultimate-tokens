...
written: 2026-09-20
amended: 2026-09-20 (revision 3: the recheck of a7a95c14 folded, 8 green, 3 yellow, 1 red, one root; see Revisions)
...
| P2 | the landing rule for every plan after #691: the baseline's test-file figure equals `TESTS.length`, by script, at the pre-land head. Read by line, not by total | `0`, then `0`: no `STALE` line names `tests`, `ui.html` or a `time`, and every stale line is a `head` line. The exit code and the `head` lines are recorded in the verdict, not graded: `exit 1` with one stale `head` line is the normal pre-land reading of a code plan | in the clone, lower the baseline's test-file figure by one: the first figure prints `1` and so does the second | both `0` on main. The `tests` line goes `STALE` when U2 registers its file and stays so until U2's last step; expected on the unit branch, a blocker at pre-land |
...
| U2-7 | the records agree | one `ramp-identity` row; then P2's two figures `0`, `0` | P2's control | `0`; P2 green at 48 |
...
One PR from `plan/gate-gaps` to `main`, title `test(color-engine): default kit in every anchor sweep, ramp identity control (#715)`. Draft at the first verified unit. Pre-land per adapter §2.1: P1 to P5 at the branch head, U1-1, U1-2 and U2-3 rerun by the pre-land verifier, record at `.sdlc/verdicts/gate-gaps-prepr.md` with the head sha. P2 is the post-#691 landing rule and is not waivable by a seat. After the squash the Orchestrator's plan-closing commit on main re-points the baseline `ref` to the landed commit, whose tree equals the pre-land head's outside `.sdlc/`, and the check then reads `stale total: 0` there.

...
- **Surface** S1. **Sites** 2: palette Enabled/Disabled (`switchControl`, `sections/color.js:1771`) and
  ...
  it is a `segmented()` `role=group` (`sections/color.js:2111`, card 3), as is its On-colors sibling
...
- **Surface** S1. **Sites** 15 static `segmented()` calls: section switcher `app.js:1415`; inspector
  ...
  hue space `sections/color.js:2111`; on-colors `sections/color.js:2122`; breakpoint mode
...
- **Surface** S1. **Sites** 3 — Distribution (`field()`, `sections/color.js:2022`), Curve (`sections/color.js:2067`),
...
- **a11y** ✓ native keyboard/picker; ✓ `aria-label` on `.map-raw-select` (`sections/color.js:1354-1356`);
  ✓ Distribution/Curve are built through `field()` (`sections/color.js:2022/2067`), which stamps an `id`
...
- **Surface** S1. **Sites** 1 — "ends bend same way" (`sections/color.js:1852-1854`, native `type=checkbox`).

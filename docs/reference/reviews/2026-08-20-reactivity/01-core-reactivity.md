...
| `_sync<X>()` ×4 | tail of every `render()` | reconcile one native `<dialog>`'s open/closed vs a ui-session boolean (Drawer/NewPal/ApplyGate/Settings) — identical pattern each time | called app.js:598-601; `_syncDrawer` app.js:610; `_syncNewPal` color.js:412; `_syncApplyGate` apply-gate.js:337; `_syncSettings` settings.js:16 |

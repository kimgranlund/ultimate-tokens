...
- Doc drift (§8, measured at `d814500`): 17 rows, 2 drifted, 15 hold, 0 undetermined.
...
| DD5 | `.claude/CLAUDE.md:111` "(local-only, ignored via `.git/info/exclude`) never reaches a commit" | fresh `git clone` of this branch at `d814500`: `git check-ignore -v .claude/docs/other/anything.md` exits 1 (no match); its `.git/info/exclude` is the untouched git template, no `docs/other` line; `.gitignore` also carries no such line | drifted: the sentence states `.git/info/exclude` as the ignore mechanism as if it were a repo property, but `info/exclude` is per-checkout and is not distributed by a clone; a fresh clone ignores nothing here by that name. What actually stops a commit is `git-precommit-privatedocs-guard` (K15), which the sentence names as a separate enforcement layer, not the ignore claim itself |
...
| DD9 | `.claude/CLAUDE.md:95` "no `node_modules`, ~60 s, tree clean after" | `.sdlc/baseline.md` Pass table, `npm test` row: `63.54 · 65.90 · 59.17` (rounded 59 to 66 s) | holds: "~60 s" is a reasonable approximation of a 59-to-66-second measured range at `d814500` |
| DD10 | `.claude/CLAUDE.md:53` "TKT-0031`, Issues #325–#342)" | `gh issue view 325 --json labels` and `gh issue view 342 --json labels`: both carry the `kind:feature`/`size:small` frontmatter-migration labels ADR-017 describes | holds: both boundary issues of the cited range exist and carry the migration labels, checked live against GitHub at `d814500` |
| DD11 | `.claude/CLAUDE.md:19` "land in `smoke-out/` (gitignored)" | `.gitignore:4` `smoke-out/` | holds: the line is present verbatim |
| DD12 | `.claude/CLAUDE.md:91` "builders and reviewers in `.worktrees/<unit>` off `plan/<slug>`" | `.gitignore:18` `.worktrees/`; this very unit's own worktree path is `.worktrees/rr-U2` | holds: the ignore rule and the naming pattern both match live practice at `d814500` |
| DD13 | `README.md:22` "the 16 default palettes (8 brand families + 8" | `src/ui/model.mjs:274-296` `DEFAULT_PALETTES`: 8 brand rows (Neutral, Primary, Secondary, Tertiary, Info, Success, Warning, Danger) + 8 `Data N` rows = 16 | holds: counted directly, 8 + 8 = 16 at `d814500` |

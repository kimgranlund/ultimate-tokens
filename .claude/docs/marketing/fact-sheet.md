...
| Export formats | CSS custom properties (HEX or OKLCH) · Tailwind v4 · shadcn/ui · DTCG · JSON · Figma variables · Figma UI3 (Material) · Claude Design bundle (`tokens.json` + `DESIGN.md` + preview components) · re-importable Config · Download-all `.zip` | `src/engine/exports.js` |
...
| Claude Design export | **free** on every tier — a bundle for **Claude Design** (`claude.ai/design`, Anthropic's surface where a vision-capable Claude generates on-brand UI from a design system): `tokens.json` (light + dark colour role set + type / spacing / radii) · `DESIGN.md` (a **9-section** generation prompt carrying the kit's own guardrails) · `components/*.html` (**7** preview cards). Surfaced as a "Claude Design" Export-drawer group (tokens.json + DESIGN.md tabs) + Download-all's `claude-design/` folder | `src/engine/exports.js` `exportClaudeDesignBundle` |
...
| Free tier | full generator · **2 brand kits** · core exports · base treatments · offline Figma plugin · MCP download · Claude Design export | `src/engine/flags.js` |

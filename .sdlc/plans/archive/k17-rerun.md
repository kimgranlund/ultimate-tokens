...
| 2026-09-20 | landed as PR #711, squash `41b2877e`, ticket #710 closed, plan archived. Pre-land record 🟢 at `092d2941` with three independent runs and CI `success` on all jobs at that sha. Convention settled for pre-land concern 7, on the adopt-hygiene precedent: the pre-land record and its reviewer record stay UNCOMMITTED on the plan branch while the gate reads them, so the `sha:` they name is the branch head, and they are committed to `main` in the close-out. Concern 3 (empty PR body) was filled through `gh pr edit` under the adapter's ruled door X4 (c), because `adapter.py land` sets a body only at PR creation and has no verb to edit one; that gap is the Conductor's to file | landing and close-out |

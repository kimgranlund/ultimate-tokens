...
- **`export_tokens(format)`**: wraps `src/engine/exports.js`, via the SAME `projectView(doc).exports`
  ...
  precedent, e.g. `get_ramp`'s "no palette …" shape) until a real generate call happens. **The
  downloadable zip does NOT grow a doc** (§12 item 3, RESOLVED), a seeded `brand-kit.json` gives the
  read surface immediately without one; `export_tokens` staying gated on a real `generate_kit` call is
  the documented, correct behavior for a downloaded package, not a gap to close.
...
- **Local:** the generator's packaging/download surface in the app is gated by
  `flagOf("describePalette")`, **RESOLVED (this build)**: `app.js#downloadDescribePaletteMcp`
  checks the flag before building the zip at all (a toast + routing a web user to Settings « Account »
  on lock, mirroring `_blockedBySetCap`'s pattern), not `_proUpsell()`'s content-panel form, this is a
  button-triggered download action, not a code-preview tab, so the action-gate shape fits better. This
  is a deliberate narrowing of the ruling's literal wording ("the local server's **generate_kit** tool"
  ...
  Confirmed by construction: the Free brand-kit-zip download (`downloadBrandKitMcp`) is a wholly
  separate function/button/zip from the Pro one, a Free user's existing download can't gain
  `generate_kit` as a side effect, because there is no shared code path between them to leak through.
...
3. ~~**Packaging**~~, **RESOLVED:** the merged server ships as a SEPARATE, standalone
   `<kit>-describe-mcp.zip` from the export drawer's Config tab, BESIDE the existing free
   `<kit>-mcp.zip` (`downloadBrandKitMcp`), it does not replace it. Rationale: the free download's
   promise ("your tokens, offline") stays intact and untouched by a Pro capability; a user who only
   wants the read-only server never sees the merged one's extra surface. `generate_kit`'s `describe-
   kit-core.mjs` needs the real engine (`model.mjs`/`persist.js`, most of `src/engine/`), not just a
   resolved `kit.json`, so `DESCRIBE_MCP_FILES` (`src/ui/describe-mcp-assets.js`, generated by
   `scripts/gen-describe-mcp-assets.mjs`) ships the REAL relative-path source tree (`mcp/` + `src/ui/`
   + `src/engine/` + `docs/reference/data/role-table.json`) rather than a bundled single file, Node
   resolves the same relative imports the repo itself uses, unmodified, so there is no second bundler
   to keep in sync with the engine's real import graph. The zip also seeds `mcp/brand-kit.json` with
   the current resolved kit, so the read tools work immediately (spec §7's "doc present" condition for
   `export_tokens` is unaffected, see that section's note). `test/mcp/describe-mcp-package.mjs`
   extracts the exact shipped file closure to a bare temp directory (nothing else from the repo beside
   it) and spawns the real server, proving the download is genuinely self-sufficient.
...
6. **Eval ops** (#375), **partially resolved**: run cadence is decided (`.github/workflows/
   ...
   **Amendment (2026-09-16).** The adopt-hygiene plan (`.sdlc/plans/archive/adopt-hygiene.md`) flips this: the

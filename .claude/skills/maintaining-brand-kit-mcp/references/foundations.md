...
- **`brandKit(doc, systems)`** (`src/ui/model.mjs:237`) is the **pure projection** that produces
  ...
  ```
  { $schema: "ultimate-tokens-brand-kit/2", name, generator: "Ultimate Tokens",
...
Two version numbers live here and are unrelated: `kit.$schema = "ultimate-tokens-brand-kit/2"` (the data shape) and
`PROTOCOL_VERSION = "2025-06-18"` (the MCP wire protocol the `initialize` reply advertises). `SERVER =
{ name: "ultimate-tokens-brand-kit", version: "0.2.0" }`. Don't conflate the schema and the protocol version.

...
| C8 | Determinism | [gate] | no RNG/clock/locale; memo caches key on the exact float; `node test/engine/hct.mjs` `oklch-deterministic` green; `node test/engine/tonal.mjs` `okl-order` green; `npm test` reproducible | 1: nondeterministic output / a cache that doesn't dedupe · 3: deterministic · 5: deterministic + a new cache keys on the exact float or does not exist |

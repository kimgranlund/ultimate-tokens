...
|---|---|---|---|---|---|
| test | `npm test` | Node 24. No `node_modules`, no browser (survey C1 🟢) | exit 0 and the runner's last line reads `all N test files passed`, where N is the length of `TESTS` in `test/run.mjs` (the last measured N is in `.sdlc/baseline.md`; a unit that adds a test file raises N and must register it in TESTS, K17). Then `git status --short` is empty: every asset `npm test` regenerates is byte-stable (survey C11 of 2026-09-18, CI drift gate). `src/ui/type-fonts.js` is outside that set, because `npm test` never runs `gen:type-fonts`; the fonts row below is its gate | 56 to 60 s baseline; **INTERIM ceiling, #681 U5, owner ruling 2026-09-20 (#713)**: 280 to 550 s wall on a host at load under about 10 - the per-file table, the full reading series and the ruling are in `.sdlc/baseline.md` §Interim gate-time ceiling; see also the note below the table | builder every pass; verifier every verdict; CI |
...
| corpus-contrast | `npm run gate:corpus-contrast` | Node 24. No `node_modules`, no browser; reads the committed `src/ui/categories/*.js` mirrors, so run it after `npm test` has regenerated them | exit 0 and the last line reads `PASS: every measured curated preset's accent clears 4.5:1 against its own on-color`, over the full curated corpus in all three tone modes (#674; the measured document, palette and cell counts are in `.sdlc/baseline.md`). `npm test` already runs the same gate SAMPLED (brands.json in full plus one deterministic volume per gallery category), so this is the full-corpus leg only | 20 to 23 s | CI (`corpus-contrast` job); builder or verifier when a unit touches the tonal ramp, `src/engine/semantic.js`, an on-color policy, or a category spec |
| fonts | `npm run gen:type-fonts` | nothing | tree clean after (K9 exception: `npm test` never runs this generator) | 1 to 1 s | builder whenever a bundled font changes, in the same commit |
| ceiling-counts | `node .sdlc/checks/ceiling-counts-check.mjs` | Node 24. Nothing else; it reads two files and writes none | exit 0 and the last line reads `ceiling-counts: clean`. It re-derives the reading counts and the graded/explicit/unsupportable partition of `.sdlc/baseline.md` §Interim gate-time ceiling from the rows themselves and checks the section's prose and this file's pointer at it against them, one line per assertion. Red means a record disagrees with its own table, not that anything is broken in the product | under 1 s | Any seat that edits the ceiling section or the budget note in §3, and the pre-land pass |
...
**Amendment (2026-09-17).** U3 set the repository to squash-merge only (`allow_merge_commit` and `allow_rebase_merge` false, ruling DP1, debt P1 until records-followup U7), so the Merge row's "not enforced" no longer holds; `main` still has no branch protection.

**Amendment (2026-09-19).** Since #674 a PR runs three jobs, `build-test`, `panda-smoke` and `corpus-contrast`, all of which must report `success`; `deploy` runs on a main push only. The "Two jobs" row above it is history and stays.
...
**Amendment (2026-09-17).** U2 added `.worktrees/` to `.gitignore` and to `SKIP_DIRS` in `test/repo/branding.mjs`, so a root-checkout `npm test` no longer walks nested unit worktrees; U3 untracked the seven `.claude/ops/` files (X6) and set the GitHub repo to squash-merge only (DP1, debt P1 until records-followup U7).
...
**Amendment (2026-09-19).** Verbatim-quote rule (ticket #709, pre-land ruling 2 on records-refresh). Text a program printed is quoted byte for byte, glyphs included, inside an inline backtick span, one span per line of output. The no-em-dash rule is a prose rule and measures prose: a plan's added-line count strips backtick spans first (`perl -pe 's/\x60[^\x60]*\x60//g'`), so a verbatim quote never trips it and is never reworded to pass it. The strip exempts every backtick span, a command or a path as much as a quote, so it is not the only control: each plan also counts the added dashes raw against the unit's merge base, the unit's own handoff excluded, and states the expected number as the dashes the enumerated quoted lines carry, counted from the program's output and never from the diff, re-derived for every unit; a dash a span smuggled into prose shows there as one above the count. If a quote has to change (cut short, a banned string removed, or a glyph the file cannot hold such as an ANSI escape or a control character, written in its place as its name or hex code), the cell says `altered:` and what changed, next to the quote. Leading whitespace the program printed is kept byte for byte, or its loss is marked the same way (`altered: two leading spaces dropped`). The branding gate (`test/repo/branding.mjs`, row X12) has no quote exemption and gets none: a quote that would carry one of its three banned strings is cut before that string and marked `altered: retired name removed`. Copy a quote from the program's output or its source line, never from another record. The strip reads inline spans only, so a quote goes in a span, one per line of output, never in a fenced block; a plan that must fence output strips the fence in its count command and says so. The rule binds lines added from 2026-09-19 on; an earlier quote is restored when a plan touches its record, and records-followup U6 lists the ones known.

**Amendment (2026-09-19).** Cited-quote clause of the verbatim-quote rule (records-followup U3, from U6 review 1). A quote cited as the subject of a finding is not a carrier: a verdict, review or corrections row that names a defective quotation writes it as the record holds it, marks the span `cited:` next to it, and no count, sweep or restoration reads that span as a quote of the program. The rule sentence on copying from the program governs a quote offered as evidence of what a program prints, not a quote offered as evidence of what a record says. A byte-pinned verbatim copy of another seat's record, taken into `.sdlc/verdicts/` and pinned by a hash its own header states, whose header also says which bytes that hash covers and names the seat whose record it is and the head that seat graded at, so that a pin is traceable to its author and no seat can pin its own text and claim the exemption, is exempt from the sweep and from every restoration and marking, and is kept byte for byte, because it cannot both keep the hash that proves it faithful and be rewritten (records-followup U10, plan revision 12).

...
priority: P1
lane: tooling
size: S + M + M + S + S + M (U1 S = 1 point, U2 M = 2, U3 M = 2, U4 S = 1, U5 S = 1, U6 M = 2; 9 points)
...
| P2 | the ceiling holds in the figures of record | `grep '^[\|] .npm test. [\|]' .sdlc/baseline.md \| awk -F'[\|]' '{n=split($5,t,"·"); m=0; for(i=1;i<=n;i++) if (t[i]+0>m) m=t[i]+0; print (n==3 && m<=120) ? "ok " m : "OVER " m}'` | `ok` and a number at or under 120 (pre-land, after U6) | in the clone, change one of the three figures to `121.00`: prints `OVER 121` | `ok 59.83` on main, which predates #681 and proves only that the command parses. At #681's landing the interim row reads near 300 and this prints `OVER` |
| P3 | coverage is not given back: every gate script runs its sweep on everything | `for g in corpus-tonal corpus-anchor sweep-prime corpus-reset; do npm run -s gate:$g > "$F/$g.log" 2>&1; echo "$g exit $?"; grep -c -e '(FULL: 343 curated documents, 3780 palettes)' -e '(FULL: 2000 determinism cases, 1500 poison renders, hue step 1)' "$F/$g.log"; done` | four times `exit 0` then `1` (three times if Q1 is no) | in the clone, delete ` --full` from one script in `package.json`: that gate's count prints `0`, because the file prints the SAMPLED line. Second control, per file: in the clone, feed the FULL branch the sampled list; the file's own vacuity check fails it (`FAIL`, exit 1), so a gate that says FULL cannot be running on less | no `--full` flag exists; all four scripts are absent, `npm run gate:corpus-tonal` exits 1 with `Missing script` |
| P4 | CI invokes every gate script, on every PR, and a red leg fails the run | `for g in corpus-tonal corpus-anchor sweep-prime corpus-reset corpus-contrast; do printf "$g "; grep -c "gate:$g" .github/workflows/ci.yml; done; grep -c '^  sweeps:$' .github/workflows/ci.yml; awk '/^  sweeps:/,/^  [a-z-]+:$/ && !/^  sweeps:/' .github/workflows/ci.yml \| grep -cE '^\s+(if:\|continue-on-error:\|needs:)'; grep -c 'fail-fast: false' .github/workflows/ci.yml; grep -cE '^\s+pull_request:' .github/workflows/ci.yml` | each name `1` or more, then `1`, `0`, `1`, `1` | in the clone, delete the `corpus-anchor` matrix entry: its line prints `0`. Add `continue-on-error: true` under `sweeps:`: the figure after the job count prints `1` | `0 0 0 0 1`, then `0`, `0`, `0`, `1` (run by the planner at 3ce50daa) |
| P5 | every moved sweep still reds on its mutation, in the FULL leg | in five clones, apply M-A to M-E as written above, check the `git diff --stat` line, then run the gate scripts named in the mutation table and grep the cut red line | each run `exit 1` and the red line present: M-A tonal `(iv dip gate) peak:`, M-A anchor `lone-spike allow-list`, M-B prime `out-of-gamut rungs exceeds the pinned ceiling` and a determinism line whose first number is not `0`, M-C tonal `(C6 i)`, M-C anchor `anchor-ramp monotone:`, M-D reset `(rst-corpus)`, M-E anchor `hueSpace-perceptual-bound` | the control of the control: the same seven runs with no mutation applied all exit 0 (that is P3) | all seven red today on the unsplit files, values in the mutation table |
...
| P8 | branding clean, and no added line carries an em dash outside a backtick span | `bash -c 'set -o pipefail; node test/repo/branding.mjs \| tail -1'; git diff $(git merge-base origin/main HEAD) \| grep '^+[^+]' \| perl -CSD -ne 's/`[^`]*`//g; print if /\x{2014}/' \| wc -l` | `branding: clean (N files scanned)`, then `0` | in the clone: `cp docs/reference/references/decision-records.md .sdlc/verdicts/x.md` reds the first; one added line with the dash makes the second `1` | clean |
| P9 | scope wall | `git diff --name-only $(git merge-base origin/main HEAD) \| grep -vcE '^(test/engine/(tonal\|anchor\|prime\|curated-contrast\|corpus-sample)\.mjs\|test/engine/lib/corpus-sample\.mjs\|test/ui/headless-boot\.mjs\|test/run\.mjs\|package\.json\|\.github/workflows/ci\.yml\|\.sdlc/\|\.claude/skills/shipping-changes/SKILL\.md\|\.claude/CLAUDE\.md)'; git diff --name-only $(git merge-base origin/main HEAD) -- src \| wc -l` | `0`, `0`. Generated files do not appear because nothing under `src/` or `scripts/` changes | `echo "// probe" >> src/engine/hct.js` in the clone: `1`, `1` | `0`, `0` |
...
Grades come from the Orchestrator's table: L1 and L2 builders get reviewer-l1 and verifier-l1; L3 and L4 get reviewer-l2 and verifier-l2. Order: U1, then U2 to U5 in any order or in parallel (they touch different files), then U6.
...
| U6 CI matrix job, figures of record, adapter, baseline, check script, shipping skill | M | builder-l3 | reviewer-l2 | verifier-l2 | `.github/workflows/ci.yml`, `.sdlc/adapter.md`, `.sdlc/baseline.md`, `.sdlc/checks/baseline-agrees-check.sh`, `.claude/skills/shipping-changes/SKILL.md`, `.claude/CLAUDE.md` (Q2) |

U2 and U3 are L4 because the pinned-list semantics are easy to get subtly wrong: a sampled leg that still demands every cited name reds forever, and one that stops checking unlisted names is vacuous. U4 is L3 because it touches the #686 guard. Every unit's step 1 is G0. Every unit runs P1, P8 and P9; U2 to U5 also run their rows of P3 and P5.
...
| U1-3 | exactly one picker exists | `grep -lE 'fnv1a\|% vols\.length' test/engine/*.mjs test/engine/lib/*.mjs test/ui/*.mjs \| sort` | one line: `test/engine/lib/corpus-sample.mjs` | restore the old `pickVolume` in `curated-contrast.mjs`: two lines | one line, `test/engine/curated-contrast.mjs` |
| U1-4 | `curated-contrast` is green in both modes on the shared picker, and its sampled line names the sorted pick | `node test/engine/curated-contrast.mjs \| tail -4 \| grep -c 'architecture V, cuisine III, film IX, literature V, music IV, nature X, travel XI'; npm run -s gate:corpus-contrast \| tail -1` | `1`, then `PASS: every measured curated preset's accent clears 4.5:1 against its own on-color` | in the clone, restore the unsorted pick: the first figure prints `0` | `0` (today's line reads VI, III, V, VI, IV, X, XI), then the PASS line |
| U1-5 | the five scripts exist and carry the flag | `node -e 'const s=require("./package.json").scripts; for (const k of ["gate:corpus-tonal","gate:corpus-anchor","gate:sweep-prime","gate:corpus-reset","gate:sweeps"]) console.log(k, /--full\|gate:/.test(s[k]\|\|"") ? "ok" : "MISSING")'` | five `ok` (four, without `gate:corpus-reset`, if Q1 is no) | delete one: `MISSING` | five `MISSING` |
...
| # | Criterion | Command | Expected | Negative control | Today |
|---|---|---|---|---|---|
| U2-1 | FULL is the old file: same gates, same verdicts, full corpus | `npm run -s gate:corpus-tonal > "$F/t.log" 2>&1; echo "exit $?"; grep -c '^  pass  ' "$F/t.log"; grep -c '^  FAIL' "$F/t.log"; grep -c '(FULL: 343 curated documents, 3780 palettes)' "$F/t.log"` | `exit 0`, `19`, `0`, `1` | P5's M-A and M-C rows for tonal | `node test/engine/tonal.mjs`: exit 0, 19 pass lines, 0 FAIL lines, no mode line (run by the planner) |
...
| U3-4 | subset semantics, both halves | as U2-4 (a) and (b), on the `gap-19` allow-list | (a) `exit 1`. (b) sampled `exit 0`, FULL `exit 1` | each other | FULL only |
...
| U4-3 | the #686 tripwire still trips locally | M-B clone: `node test/engine/prime.mjs > "$F/psb.log" 2>&1; echo "exit $?"; grep -E '[0-9]+/400 palettes' "$F/psb.log"` | `exit 1`. The determinism count is recorded; at the measured 7 in 2000 a 400-case prefix expects 1 or 2 and may read 0, so the graded red is the exit code, carried by the grids: 409 of 151200 rungs under FULL, so about 80 of 30240 at hue step 5 by proportion (not measured) | U4-2 | FULL: 7/2000, 409/151200 |
...
| U5-3 | only the sweep block changed | `git diff $(git merge-base plan/gate-split HEAD) -- test/ui/headless-boot.mjs \| grep '^[-+][^-+]' \| grep -vcE 'corpus\|FULL\|sample\|SAMPLED\|--full\|import'` | `0` | edit any lettered group in the clone: 1 or more | not applicable |
| U5-4 | the file's share | as U2-5 | at or under 30 s quiet | none | 93 to 104 s loud, 61.0 s Lane A |

### U6: CI and the records

Steps. (1) G0, and U1 to U5 merged. (2) Add the `sweeps` matrix job. (3) Under the quiet-host rule, run `npm test` three times and each gate script three times; the handoff's Runs table is `| # | command | load before | hot before | pgrep before | load after | hot after | exit | wall (s) | last line | git status lines |`. A run that fails the rule is recorded and rerun later, never averaged in. If no quiet slot arrives the builder says so and the unit waits; it does not grade a loud figure. (4) `baseline.md`: new `ref`, the three `npm test` figures and N, one Pass row per gate script, the #681 interim row moved to a labelled prior set. (5) Adapter §1: the test row's time range and a sentence stating the 120 s ceiling and the quiet-host rule; one row per gate script (command, needs, what green means including the FULL line, time range, who runs it: CI always, builder or verifier when a unit touches what the row names); the #681 interim budget note retired. §2 "CI on a PR": every job in `ci.yml` named. (6) `baseline-agrees-check.sh`: one `time <gate>` line per gate script, same rounding rule as the three it has. (7) `shipping-changes` SKILL.md lines 32, 54 and 116: the job list and the watch time. (8) If Q2 is yes, `.claude/CLAUDE.md:99` names the jobs.

| # | Criterion | Command | Expected | Negative control | Today |
|---|---|---|---|---|---|
| U6-1 | P4 | as P4 | as P4 | as P4 | as P4 |
...
| U6-3 | nine or more counted runs, all quiet, all green, tree clean | `HF=.sdlc/handoffs/gate-split-U6.md; grep -cE '^[\|] [0-9]+ [\|] .npm (test\|run gate:)' $HF; grep -E '^[\|] [0-9]+ [\|] .npm' $HF \| awk -F'[\|]' -v c="$(sysctl -n hw.ncpu)" '{for(i=4;i<=8;i++) gsub(/ /,"",$i); if ($4+0>=c \|\| $7+0>=c \|\| $5!="0" \|\| $8!="0" \|\| $6!="none" \|\| $9+0!=0 \|\| $12+0!=0) n++} END{print n+0}'` | `15` (3 x `npm test` plus 3 x 4 gates; `12` if Q1 is no), then `0` | plant a row with load `11.2` or hot `2`: second figure `1` | no handoff |
| U6-4 | P2 | as P2 | as P2 | as P2 | as P2 |
...
| U6-6 | adapter §1 carries every gate script, the ceiling and the rule; the interim note is gone; §2 names every CI job | `for g in corpus-tonal corpus-anchor sweep-prime corpus-reset; do grep -c "^[\|] [a-z-]* [\|] .npm run gate:$g. [\|]" .sdlc/adapter.md; done; grep -c '120 s' .sdlc/adapter.md; grep -c "pcpu" .sdlc/adapter.md; grep -c 'budget moved, #681' .sdlc/adapter.md; for j in $(perl -ne '$j=1 if /^jobs:/; print "$1\n" if $j && /^  ([a-z-]+):$/' .github/workflows/ci.yml); do grep '^[\|] CI on a PR' .sdlc/adapter.md \| grep -c "$j"; done` | four `1`, then `1` or more, `1` or more, `0`, then one `1` per job | drop a row in the clone: its `0` shows | four `0`, `0`, `0`, `0` on main (`1` once #681 lands), then `1 1 0 1` for `build-test`, `panda-smoke`, `corpus-contrast`, `deploy` (run by the planner) |
| U6-7 | the shipping skill names the same jobs | `grep -c 'gate:sweeps' .claude/skills/shipping-changes/SKILL.md; grep -c 'all three legs' .claude/skills/shipping-changes/SKILL.md` | `1` or more, `0` | revert the file in the clone: `0`, `1` | `0`, `1` |
| U6-8 | P10, once the draft PR has a completed run | as P10 | as P10 | as P10 | as P10 |
...
| The 120 s ceiling does not hold on a quiet host | It is arithmetic from loud measurements. The knob is the sample: prime's 400 cases and the hue step. If three quiet runs exceed 120 s with the stated sample, U6 stops and writes `.sdlc/questions/gate-split-U6.md` with the per-file table; it does not raise the ceiling or shrink the sample on its own |
| No quiet slot ever arrives on this host | U6 waits and says so. The CI figures (P10) do not depend on this host, so coverage and the CI budget can be verified while the local figures of record wait. Landing without them is the owner's call (Q4 is not asked now; it is asked only if it happens) |

...
Correction (2026-09-20, plan records-followup U10, #709): three spans in this handoff quote a defective form as the subject of a finding or count it with a grep, and each now carries the `cited:` marker directly after the span it governs: the hyphen-minus smoke form in the enumeration above; in criteria row 2, the `refs-canonical, ordered` `cited:` form inside that row's last grep span; in criteria row 3, the four altered smoke forms inside that row's own `git grep -c -e ...` span, whose escaped-pipe `wc -l` tail is part of that same span and not a span of its own (measured: the row holds twelve spans and the marker is the fifth of them, directly after the fourth, which is that whole command; row 2 above holds thirteen spans and its marker is the seventh, directly after the sixth). Rule: `.sdlc/adapter.md` §3, the cited-quote clause under the Verbatim-quote rule.

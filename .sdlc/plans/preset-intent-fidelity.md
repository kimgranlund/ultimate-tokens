...
- [ ] U3 (M) anchor-centred chroma envelope, all modes · grade l4
  `src/engine/tonal.js` (`chromaEnvelope`, both paths, keyed on `liftStop(stop, lift)` (the #648 displacement helper, the same call `effStop` makes before skew's gamma), never on the nominal stop and never on a separately re-derived effective stop: Lane A's #668 R1 grid evidence shows a re-derived `effStop` breaks the lift-0 control, drops Danger below its floor, moves Panda literals, and still leaves 4 grid cells rising, while `liftStop()` avoids all four; `chromaFloor` as an envelope floor, saturation basis = the key colour's own OKHSL `s` for non-anchored palettes), `scripts/gen-categories.mjs` (`VIVID_MIDS.dampAmp` 55 → 0, Q7 ruled: the envelope is normalised at 500 so a non-zero `dampAmp` can only raise the shoulders, which C6 forbids above 75%; the builder reports the C6 table at both values and ships 0), `test/engine/tonal.mjs` (envelope gate C6/C7; regenerate `fixtures/tonal-legacy.json` via `scripts/gen-tonal-fixture.mjs` with the rationale in the fixture header), `test/engine/semantic.mjs` (`hpg-role-contrast` floors re-pinned, C8). Depends on U1 (anchor stop) and on #668 R1 (Lane A, landing now); can be built in parallel with U2 behind the `anchorStop` parameter. Named cases for C6 (measured on bf2aaf6, `baseline-6-668.mjs`, output `baseline-6-out.txt`): (i) perceptual: the 11 #668 palettes whose measured CIELAB L* rises at the 750 to 800 step, worst +0.5105 (travel "34° S San Telmo" secondary-muted, lift −40; then travel "59° N Lake Baikal corridor" primary-muted +0.3784; travel "67° N Helsinki–Rovaniemi night train" tertiary-muted; film "Touch of Evil" secondary, "2001: A Space Odyssey" tertiary-muted, "Arrival" primary-muted, "Blade Runner" secondary, "John Wick" tertiary-muted; music "Black metal · the forest at night" secondary; travel "35° N Yamanote line" tertiary-muted; travel "16° N Bogyoke Aung San Market" secondary, lift −34, +0.0245); (ii) peak, the unrecorded mirror of the same defect: 43 palettes, worst +0.8312 (travel "59° N Lake Baikal corridor" primary-muted), 37 of them at stop 800 with lift ≤ −30 and 6 at stop 250 with lift ≥ 32 (travel "41° N La Boqueria" secondary-muted +0.1543 the worst of those); (iii) even: 0 today. With each palette's pre-plan lift applied to a non-anchored copy, the report prints per mode the count of palettes with any measured L* rise and the worst rise; pass is 0 / 0 / 0 (today 11 / 43 / 0; #668 R1 lands first and U3 must keep all three at 0). (iv) No new duplicate hex: the report prints the per-mode count of ramps with two stops sharing a hex over the 3,780 palettes; today 0 / 0 / 0 on bf2aaf6 and pass is 0 / 0 / 0 (the unfixed peak residue on Lane A's branch produces one, which C5 for the anchored ramp and #648's rule for every ramp both forbid).

...
- `.claude/skills/ultimate-tokens-brand-voice/scripts/store-drift-check.mjs`, `PRODUCT_NAME`: was `Ultimate Tokens`, an em dash, then `Pro` (and the same for `Studio`), now `Ultimate Tokens, Pro` and `Ultimate Tokens, Studio`. `DESC_PROBES`: was `derived`, an em dash, then `not guessed`, now `derived, not guessed`.
...
3. Keep the live store's em dash name. The check's needle uses the backslash-u2014 escape (as `voice-check.mjs` does at 0faca70d), and `store-copy.md` names it in its own words. There is no store change, but the repo's source for the name no longer carries it verbatim.

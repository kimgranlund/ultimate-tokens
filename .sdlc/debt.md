...
| C5 | `.claude/settings.json` enables `sdlc@nonoun` but `extraKnownMarketplaces` declares no `nonoun` marketplace, so a fresh clone cannot resolve the plugin | `.claude/settings.json` `enabledPlugins` | `.sdlc/questions/adopt-hygiene-marketplace.md` (default A: leave it user-scoped, no public source to point at) | S | human | either declare a source in `extraKnownMarketplaces` or accept the marketplace as user-scoped on the maintainer's machine; not a code change |
| C6 | machine-local absolute home paths appear in `.sdlc/` records (`.sdlc/plans/adopt-hygiene.md`, `.sdlc/tickets/T-0001.md`, `.sdlc/architecture.md`) | `git grep -n '/Users/' -- .sdlc` | `.sdlc/verdicts/adopt-hygiene-prepr.md` config-and-records row (🟡 accepted) | S | accepted | seat-local records written on one maintainer's machine; not worth rewriting for a single-maintainer repo |

# Every file under .sdlc/verdicts/ carries a machine-readable verdict: line (#723): its LAST
# `verdict:` line's first token is one of 🟢 🟡 🔴, the line adapter.py's read_gate reads (it
# keeps the last match on every gate line). A file named in verdict-frontmatter-grandfather.txt
# is not graded, so the backfill ticket (U2) clears the mandate one file at a time.
...
for (const name of names) {
  if (!fileSet.has(name)) say(`STALE ${name}: grandfathered but absent`);
}
...
for (const f of files) {
  ...
  if (!verdictLines.length) { say(`MISSING ${f}: no verdict: line`); continue; }
  const last = verdictLines[verdictLines.length - 1];
  const value = last.slice("verdict:".length).trim();
  const token = value.split(/\s+/)[0];
  if (!["🟢", "🟡", "🔴"].includes(token)) say(`VALUE ${f}: last verdict: ${value} is not 🟢, 🟡 or 🔴`);
}

...
| U1-6 | the sampler's order is total: ties on category plus name are broken by a stated third key, and a uniqueness assertion fails loudly on a duplicate key | the builder names the command in the handoff; it prints the key count and the distinct key count | the two counts are equal, exit 0 | a fixture with two entries sharing category and name: without the tiebreak the pick differs under reversal; with a planted duplicate key the assertion exits non-zero |
